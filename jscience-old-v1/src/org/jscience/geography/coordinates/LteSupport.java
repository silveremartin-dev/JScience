...
/**
 @author David Shen
 @brief class implementing the suport functions specifically for the LTE creation and
...
class LteSupport {
     ...
     * DOCUMENT ME!
     *
     * @param srf_org DOCUMENT ME!
     * @param coord_org DOCUMENT ME!
    ...
    protected static SRF_LocalTangentSpaceEuclidean createLtesSRF(
        BaseSRF srf_org, double[] coord_org, //surface coord
        double azimuth, double x_false_origin, double y_false_origin,
        double height_offset) throws SrmException {
        SRF_LTSE_Params my_new_lte;
        SRF_Celestiodetic cd_srf_obj;
        double[] my_3D_org = { coord_org[0], coord_org[1], 0.0 };
        double[] my_3D_tgt = new double[3];

        // First create Celestiodetic SRF using the same Orm/Hsr
        cd_srf_obj = new SRF_Celestiodetic(srf_org.get_orm(), srf_org.get_hsr());

        // Convert the source coordinate ( 0.0, 0.0 ) to Celestiodetic
        OpManager.instance()
                 .computeAsArray(srf_org, cd_srf_obj, my_3D_org, my_3D_tgt);

        // Instantiate the LTE SRF using the lon and lat obtained from the above conversion
        return new SRF_LocalTangentSpaceEuclidean(srf_org.get_orm(),
            srf_org.get_hsr(), my_3D_tgt[0], my_3D_tgt[1], azimuth,
            x_false_origin, y_false_origin, height_offset);
    }

    // conputes the LTE rotation matrix F.
    /**
     * DOCUMENT ME!
     *
     * @param T DOCUMENT ME!
     ...
     */
    protected static void LTP_Rotation_Matrix_F(double[][] T, double lambda, // lon
        double phi, // lat
