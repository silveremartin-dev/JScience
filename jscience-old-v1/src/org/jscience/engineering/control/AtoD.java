...
*           Revised: 18 August 2003, 5 May 2005
*
*           DOCUMENTATION:
...
public class AtoD extends BlackBox {
    ...
    public AtoD(int nBits, double vRef) {
        if (nBits > 63) {
            throw new IllegalArgumentException(
                    "This program cannot accomadate an ADC simulation with a number of bits greater than 63");
        }
    ...
    public String getRange() {
        ...
        if (this.trueAtoD) {
            ...
            System.out.println(
                    "No range option set - this instance of AtoD is an 'ADC marker' only");
    ...
    public boolean getTrueAtoDoption() {
        if (this.trueAtoD) {
            System.out.println(
                    "This instance of AtoD is a true simulation of an ADC");
            System.out.println("getTrueAtoDoption has returned 'true'");
        } else {
            System.out.println(
                    "This instance of AtoD is not a true simulation of an ADC");
    ...
    public double getVref() {
        if (!this.trueAtoD) {
            System.out.println(
                    "No reference voltage set - this instance of AtoD is an 'ADC marker' only");
    ...
    public void checkInput() {
        ...
        if (this.trueAtoD) {
            if (this.range) {
                ...
                if (this.input < -this.vRef) {
                    System.out.println("lower limit of the ADC range exceeded");
                    System.out.println("input voltage set to " +
                            (-this.vRef / 2));
    ...
    public void calcOutput() {
        if (this.trueAtoD) {
            this.decimalOutput = (long) (Math.floor(((this.inputC) / this.vRef) * this.maximumDecimal)) -
                    this.decimalShift;
            this.voltageOutput = (this.vRef * this.decimalOutput) / this.maximumDecimal;
            this.sqnr = 20.0D * MathUtils.log10(Math.abs(
                    (this.inputC - this.shift) / (this.inputC - this.shift -
                            this.voltageOutput)));
        } else {
            this.voltageOutput = this.input;
            this.sqnr = 1.0D / 0.0D;  //the genuine author said this is NOT a mistake
        }

        super.sNumer.setCoefficientAsComplex(0,
                new Complex(this.voltageOutput / this.input, 0.0D));
    ...
    public long decimalOutput() {
        ...
        if (!this.trueAtoD) {
            System.out.println(
                    "No formal A to D conversion performed - this instance of AtoD is an 'ADC marker' only");
