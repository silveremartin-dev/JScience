...
*           Revised: 18 August 2003, 9 May 2005
*
*           DOCUMENTATION:
...
public class DtoA extends BlackBox {
    ...
    public boolean getTrueDtoAoption() {
        if (this.trueDtoA) {
            System.out.println(
                    "This instance of DtoA is a true simulation of an ADC");
            System.out.println("getTrueDtoAoption has returned 'true'");
        } else {
            System.out.println(
                    "This instance of DtoA is not a true simulation of an ADC");
    ...
    public void setInput(String input) {
        ...
        if (len > (this.nBits + 1)) {
            throw new IllegalArgumentException(
                    "length of input String is greater than the DAC bit number plus one");
        }

        if (len < (this.nBits + 1)) {
            System.out.println("Class - DtoA;  method - setInput(String)");
            System.out.println(
                    "The input String is less than DAC number of bits plus one");
            System.out.println(
                    "String assumed to represent a postive unsigned binary number");
        ...
        for (int i = len - 1; i >= 0; i--) {
            ...
            if (c == '1') {
                ...
                if (c == '0') {
                    ...
                    throw new IllegalArgumentException(
                            "String input must be '0's or '1's");
                }
            }
        ...
        long sign = 1L;
        int[] vPosBinary = (int[]) this.vBinary.clone();
        ...
        // convert to voltage
        this.outputVoltage = (this.decimalInput * this.vRef) / (this.maximumDecimal +
                1L);
    ...
    public void setInput(int[] input) {
        ...
        if (len > (this.nBits + 1)) {
            throw new IllegalArgumentException(
                    "length of input array is greater than the DAC bit number plus  one");
        }
        ...
        if (len < (this.nBits + 1)) {
            System.out.println("Class - DtoA;  method - setInput(String)");
            System.out.println(
                    "The input array is less than DAC number of bits plus one");
            System.out.println(
                    "Array assumed to represent a postive unsigned binary number");
            System.out.println("unfilled bits assigned zeros");
    ...
    public void setInput(long input) {
        if (Math.abs(input) > this.maximumDecimal) {
            throw new IllegalArgumentException("abs(input), " + input +
                    ", is greater than the maximum decimal representation, " +
                    this.maximumDecimal + ", allowed by the set number of bits, " +
                    this.nBits);
        }
    ...
    public void setInput(double input) {
        ...
        super.sNumer.setCoefficientAsComplex(0,
                new Complex(this.outputVoltage / this.voltageInput, 0.0D));
    ...
    public long getDecimalInput() {
        ...
        if (!this.trueDtoA) {
            ...
            System.out.println(
                    "This instance of DtoA is not a true simulation of an DAC");
            System.out.println("It is simple an 'D to A marker'");
