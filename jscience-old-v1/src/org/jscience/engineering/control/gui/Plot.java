...
public class Plot extends Canvas {
    ...
    public Plot(double[] xdata, double[] ydata) {
        ...
        if (xl != yl) {
            throw new IllegalArgumentException(
                "x-data length is not equal to the y-data length");
        }
    ...
    private void initialise(double[][] cdata) {
        ...
        for (int i = 0; i < nCurves; i++) {
            ...
            if (l1 != l2) {
                throw new IllegalArgumentException(
                    "an x and y array length differ");
            }
        ...
        for (int i = 0; i < nCurves; i++) {
            ...
            while (testlen) {
                if (l1 < 0) {
                    throw new IllegalArgumentException("curve array index  " +
                        k + ": blank array");
                }
        ...
        for (int i = 0; i < nCurves; i++) {
            ...
            if (rev == nPoints[i]) {
                ...
                for (int j = 0; j < nPoints[i]; j++)
                    cdata[k][j] = hold[nPoints[i] - j - 1];

                for (int j = 0; j < nPoints[i]; j++)
                    hold[j] = cdata[k + 1][j];

                for (int j = 0; j < nPoints[i]; j++)
                    cdata[k + 1][j] = hold[nPoints[i] - j - 1];
            }
    ...
    public void setLine(int[] lineOpt) {
        ...
        for (int i = 0; i < n; i++)
            if ((lineOpt[i] < 0) || (lineOpt[i] > 4)) {
                throw new IllegalArgumentException(
                    "lineOpt must be 0, 1, 2, 3 or 4");
            }
                ...
                if (test0) {
                    ...
                    if (rev == nPoints[i]) {
                        ...
                        if (rev == nPoints[i]) {
                            ...
                            for (int j = 0; j < nPoints[i]; j++)
                                data[i][j] = hold[nPoints[i] - j - 1];

                            for (int j = 0; j < nPoints[i]; j++)
                                hold[j] = data[2 * i][j];

                            for (int j = 0; j < nPoints[i]; j++)
                                data[2 * i][j] = hold[nPoints[i] - j - 1];

                            this.lineOpt[i] = -lineOpt[i];
                        } else {
                            System.out.println("Curve " + i +
                                " will not support interpolation");
                            System.out.println(
                                "Straight connecting line option used");
     ...
     * DOCUMENT ME!
     *
     * @param dashLength DOCUMENT ME!
     */
    public void setDashLength(int[] dashLength) {
        if (dashLength.length != nCurves) {
            throw new IllegalArgumentException("input array of wrong length");
        }
    ...
    public void setPoint(int[] pointOpt) {
        ...
        for (int i = 0; i < n; i++)
            if ((pointOpt[i] < 0) || (pointOpt[i] > 8)) {
                throw new IllegalArgumentException(
                    "pointOpt must be 0, 1, 2, 3, 4, 5, 6, 7, or 8");
            }
     ...
     * DOCUMENT ME!
     *
     * @param mpointSize DOCUMENT ME!
     */
    public void setPointSize(int[] mpointSize) {
        if (mpointSize.length != nCurves) {
            throw new IllegalArgumentException("input array of wrong length");
        }
     ...
     * DOCUMENT ME!
     *
     * @param mmin DOCUMENT ME!
     * @param mmax DOCUMENT ME!
     *
     * @return DOCUMENT ME!
     */
     ...
     * @param lowfac DOCUMENT ME!
     * @param limits DOCUMENT ME!
     */
    public static void limits(double low, double high, double lowfac,
        double[] limits) {
        double facl = 1.0D;
        double fach = 1.0D;

        if (Math.abs(low) < 1.0D) {
            facl = 10.0D;
        }

        if (Math.abs(low) < 0.1D) {
            facl = 100.0D;
        }

        if (Math.abs(high) < 1.0D) {
            fach = 10.0D;
        }

        if (Math.abs(high) < 0.1D) {
            fach = 100.0D;
        }
    ...
    public static double offset(double low, double high) {
        ...
        double sh = MathUtils.sign(high);
        double sl = MathUtils.sign(low);
        double offset = 0.0D;
        int eh = 0;
        int ed = 0;

        if (sh == sl) {
            ed = (int) Math.floor(MathUtils.log10(diff));

            if (sh == 1) {
                eh = (int) Math.floor(MathUtils.log10(high));

                if ((eh - ed) > 1) {
                    offset = Math.floor(low * Math.pow(10, -ed)) * Math.pow(10,
                            ed);
                }
            } else {
                eh = (int) Math.floor(MathUtils.log10(Math.abs(low)));

                if ((eh - ed) > 1) {
                    offset = Math.floor(high * Math.pow(10, -ed)) * Math.pow(10,
                            ed);
                }
            }
        }
    ...
    public void axesScaleOffset() {
        ...
        for (int i = 0; i < nCurves; i++) {
            for (int j = 0; j < nPoints[i]; j++) {
                ...
                if (this.errorBar[i]) {
                    this.errors[i][j] += this.data[k + 1][j];
                }
            }
        ...
        if (this.yFac != 0) {
            ...
            for (int i = 0; i < this.nCurves; i++) {
                for (int j = 0; j < this.nPoints[i]; j++) {
                    ...
                    if (this.errorBar[i]) {
                        this.errors[i][j] *= Math.pow(10, this.yFac + 1);
                    }
                }
     ...
     * @param low DOCUMENT ME!
     * @param high DOCUMENT ME!
     * @param tickval DOCUMENT ME!
     * @param tickchar DOCUMENT ME!
     *
     * @return DOCUMENT ME!
     */
    public static int ticks(double low, double high, double[] tickval,
        String[] tickchar) {
        // Find range
        int[] trunc = { 1, 1, 1, 2, 3 };
        ...
        case 1:
            ...
            tickval[0] = truncate(low, trunc[range]);

            for (int i = 1; i < nticks; i++) {
                tickval[i] = truncate(tickval[i - 1] + inc, trunc[range]);
            }
        ...
        case -1:
            ttickval = new double[nticks];
            ttickval[0] = truncate(high, trunc[range]);

            for (int i = 1; i < nticks; i++) {
                ttickval[i] = truncate(ttickval[i - 1] - inc, trunc[range]);
            }

            ttickval = ArrayMathUtils.invert(ttickval);
        ...
        case 2:
            ...
            nticks += npExtra;
            ttickval = new double[nticks];
            tickval[0] = truncate(-npExtra * inc, trunc[range]);
        ...
        case -2:
            ...
            nticks += npExtra;
            ttickval = new double[nticks];
            ttickval[0] = truncate(npExtra * inc, trunc[range]);
    ...
    public void minMax() {
        ...
        while (test) {
            if (this.minMaxOpt[ii]) {
                ...
                if (this.errorBar[ii]) {
                    this.yMin = (2.0D * this.yMin) - this.errors[ii][0];
                }

                this.yMax = this.data[(2 * ii) + 1][0];

                if (this.errorBar[ii]) {
                    this.yMax = errors[ii][0];
                }
            } else {
                ii++;
                ...
                if (ii > nCurves) {
                    throw new IllegalArgumentException(
                        "At least one curve must be included in the maximum/minimum calculation");
                }
            }
        }
     ...
     *
     * @param trim DOCUMENT ME!
     * @param xoldpoint DOCUMENT ME!
     * @param xnewpoint DOCUMENT ME!
     * @param yoldpoint DOCUMENT ME!
     * @param ynewpoint DOCUMENT ME!
     *
     * @return DOCUMENT ME!
     */
    public boolean printCheck(boolean trim, int xoldpoint, int xnewpoint,
        int yoldpoint, int ynewpoint) {
        boolean btest2 = true;
    ...
    public void graph(Graphics g) {
        ...
        if ((this.xFac == 0) && (this.xOffset == 0)) {
            g.drawString(this.xAxisLegend + xbrack1 + this.xAxisUnits +
                xbrack2, xBot - 4, yBot + 32);
        } else {
            if (this.xOffset == 0) {
                ...
                if (this.xFac == 0) {
                    g.drawString(this.xAxisLegend + ss + xoffstr + xbrack1 +
                        this.xAxisUnits + xbrack2, xBot - 4, yBot + 30);
                } else {
                    ssx = this.xAxisLegend + ss + xoffstr + xbrack1 +
                        this.xAxisUnits + xbrack3;
                    sw = fm.stringWidth(ssx);
        ...
        if ((yFac == 0) && (yOffset == 0)) {
            ...
            if (yOffset == 0) {
                ...
                if (yFac == 0) {
                    g.drawString(this.yAxisLegend + ss + yoffstr, 15, yTop -
                        25);
                    g.drawString(ybrack1 + this.yAxisUnits + ybrack2, 15,
                        yTop - 10);
                } else {
        ...
        for (int ii = 0; ii < this.xTicks; ii++) {
            xt = xBot +
                (int) (((this.xAxisNo[ii] - (double) xLow) / xdenom) * xLen);
        ...
        for (int ii = 1; ii < this.yTicks; ii++)
            if (yAxisChar[ii].length() > yCharLenMax) {
                yCharLenMax = yAxisChar[ii].length();
            }
        ...
        for (int ii = 0; ii < this.yTicks; ii++) {
            ...
            yt = yBot -
                (int) (((this.yAxisNo[ii] - (double) yLow) / ydenom) * yLen);
        ...
        for (int i = 0; i < this.nCurves; i++) {
            ...
            if ((lineOpt[i] == 1) || (lineOpt[i] == 2)) {
                DoubleCubicSplineInterpolator cs;
                ...
                cs = new DoubleCubicSplineInterpolator(xcs, ycs);
                //cs.calcDeriv();
                xoldpoint = xBot +
                    (int) (((xcs[0] - (double) xLow) / xdenom) * xLen);
                yoldpoint = yBot -
                    (int) (((ycs[0] - (double) yLow) / ydenom) * yLen);

                for (int ii = 1; ii < niPoints; ii++) {
                    xnewpoint = xBot +
                        (int) (((ics[ii] - (double) xLow) / xdenom) * xLen);
                    ynewpoint = yBot -
                        (int) (((cs.map(ics[ii]) - (double) yLow) / ydenom) * yLen);
                    btest2 = printCheck(trimOpt[i], xoldpoint, xnewpoint,
                            yoldpoint, ynewpoint);
                    ...
                    if (btest2) {
                        ...
                        if (dcheck) {
                            g.drawLine(xoldpoint, yoldpoint, xnewpoint,
                                ynewpoint);
                        }
                    }
            ...
            if ((lineOpt[i] == -1) || (lineOpt[i] == -2)) {
                ...
                cs = new DoubleCubicSplineInterpolator(ycs, xcs);
                //cs.calcDeriv();
                xoldpoint = xBot +
                    (int) (((xcs[0] - (double) xLow) / xdenom) * xLen);
                yoldpoint = yBot -
                ...
                for (int ii = 1; ii < niPoints; ii++) {
                    ynewpoint = yBot +
                        (int) (((ics[ii] - (double) yLow) / ydenom) * yLen);
                    xnewpoint = xBot -
                        (int) (((cs.map(ics[ii]) - (double) xLow) / xdenom) * xLen);
                    btest2 = printCheck(trimOpt[i], xoldpoint, xnewpoint,
                            yoldpoint, ynewpoint);
            ...
            if (lineOpt[i] == 3) {
                ...
                dcheck = true;
                xoldpoint = xBot +
                    (int) ((((this.data[kk][0]) - (double) xLow) / xdenom) * xLen);
                yoldpoint = yBot -
                    (int) ((((this.data[kk + 1][0]) - (double) yLow) / ydenom) * yLen);

                for (int ii = 1; ii < nxpoints; ii++) {
                    xnewpoint = xBot +
                        (int) ((((this.data[kk][ii]) - (double) xLow) / xdenom) * xLen);
                    ynewpoint = yBot -
                        (int) ((((this.data[kk + 1][ii]) - (double) yLow) / ydenom) * yLen);
            ...
            if (lineOpt[i] == 4) {
                ...
                for (int ii = 1; ii < nxpoints; ii++) {
                    ...
                    ynewpoint = yBot -
                        (int) ((((this.data[kk + 1][ii]) - (double) yLow) / ydenom) * yLen);
                    lengths[ii - 1] = (int) MathUtils.hypot((double) (xnewpoint -
                            xoldpoint), (double) (ynewpoint - yoldpoint));
                    totalLength += lengths[ii - 1];
                    gradients[ii - 1] = (double) (ynewpoint - yoldpoint) / (double) (xnewpoint -
                        xoldpoint);
                    intercepts[ii - 1] = (double) yoldpoint -
                        (gradients[ii - 1] * xoldpoint);
                    xoldpoint = xnewpoint;
                    yoldpoint = ynewpoint;
                }
                ...
                for (int ii = 1; ii < nxpoints; ii++) {
                    ...
                    for (int jj = nlpointsold + 1;
                            jj < (nlpointsnew + nlpointsold); jj++)
                        totalLpoints++;
                ...
                for (int ii = 1; ii < nxpoints; ii++) {
                    nlpointsnew = lengths[ii - 1] / incrmt;
                    xdashed[nlpointsnew + nlpointsold] = xBot +
                        (int) ((((this.data[kk][ii]) - (double) xLow) / xdenom) * xLen);
                    ydashed[nlpointsnew + nlpointsold] = yBot -
                        (int) ((((this.data[kk + 1][ii]) - (double) yLow) / ydenom) * yLen);

                    if (Math.abs(gradients[ii - 1]) > 0.5) {
                        int diff = (ydashed[nlpointsnew + nlpointsold] -
                            ydashed[nlpointsold]) / nlpointsnew;

                        for (int jj = nlpointsold + 1;
                                jj < (nlpointsnew + nlpointsold); jj++) {
                            ydashed[jj] = ydashed[jj - 1] + diff;

                            if (isInfinity(Math.abs(gradients[ii - 1]))) {
                                xdashed[jj] = xdashed[nlpointsnew +
                                    nlpointsold];
                            } else {
                                xdashed[jj] = (int) (((double) ydashed[jj] -
                                    intercepts[ii - 1]) / gradients[ii - 1]);
                            }
                        }
                    } else {
                        int diff = (xdashed[nlpointsnew + nlpointsold] -
                            xdashed[nlpointsold]) / nlpointsnew;

                        for (int jj = nlpointsold + 1;
                                jj < (nlpointsnew + nlpointsold); jj++) {
                            xdashed[jj] = xdashed[jj - 1] + diff;
                            ydashed[jj] = (int) ((gradients[ii - 1] * ydashed[jj]) +
                                intercepts[ii - 1]);
                        }
                    }
                ...
                for (int ii = 1; ii < totalLpoints; ii++) {
                    ...
                    if (dcheck) {
                        g.drawLine(xdashed[ii - 1], ydashed[ii - 1],
                            xdashed[ii], ydashed[ii]);
                    }
                }
            }
                ...
                for (int ii = 0; ii < nxpoints; ii++) {
                    ...
                    case 6:
                        ...
                        for (int jj = 0; jj < psh; jj++)
                            g.drawLine(xxp - jj, yyp - psh + jj, xxp + jj,
                                yyp - psh + jj);

                        for (int jj = 0; jj <= psh; jj++)
                            g.drawLine(xxp - psh + jj, yyp + jj,
                                (xxp + psh) - jj, yyp + jj);
                    ...
                    if (this.errorBar[i]) {
                        yype = yBot -
                            (int) (((errors[i][ii] - (double) yLow) / ydenom) * yLen);
     ...
     * @return DOCUMENT ME!
     */
    private static boolean isInfinity(double x) {
        boolean test = false;
