...
public class TaskList extends CompileTimeObject {
    /**
     * This variable represents an empty task list. All over the code,
     * whenever any object needs an empty task list, it will use this variable
     * rather than create a new empty task list to save memory because
     * functionally all the empty task lists are the same.
     */
...
/**
     * To initialize this task list as an empty one (i.e., an ordered
     * non-atomic task list of length 0). Note that this is a private function
     * in order to minimize its use. In case an empty task list is needed,
...
/**
     ...
     *
     * @param taskIn the task atom this task list will represent.
     */
...
/**
     ...
     *
     * @param size      the number of task lists in the task list this object
     *                  represents.
     * @param orderedIn whether or not this task list is ordered.
     */
    ...
    /**
     * To create a non-atomic task list. This is defined as a static
     * function rather than a constructor because it returns
     * <code>TaskList.empty</code> if the required task list is of length 0
     * rather than creating a new empty task list. Other than that, this
     * function acts like a constructor.
     *
     * @param subtasksIn a <code>Vector</code> of subtasks the output task list
     *        will represent.
     * @param orderedIn whether or not the output task list should be ordered.
     *
     * @return the resulting task list.
     */
    ...
    /**
     * To return a <code>LinkedList</code> of the task atoms we have
     ...
     *
     * @return a <code>LinkedList</code> of the task atoms we have the option
    ...
    /**
     * The helper function to calculate a <code>LinkedList</code> of
     * the task atoms we have the option to achieve right now.
     *
     * @param res this <code>LinkedList</code> stores the task atoms we have
     *        incrementally calculated so far.
     *
     * @return <code>true</code> if there can be no more task atoms we have the
     ...
     *         more options in case an immediate task atom has been
     *         encountered.
     */
    ...
    /**
     * This function produces Java code used to create this task list
     * either as an atomic task list or recursively, as a list of other task
     * lists.
     *
     * @param what the <code>String</code> name of the task list created by
     *        this piece of code.
     *
    ...
    public String getInitCode(String what) {
        ...
        //-- Atomic task list.
        if (subtasks == null) {
            return "\t\t" + what + " = new TaskList(" + task.toCode() + ");" +
            endl;
        }
        ...
        s = "\t\t" + what + " = new TaskList(" + subtasks.length + ", " +
            ordered + ");" + endl;
     ...
     *
     * @return the task atom associated with this task list in case this is an
    ...
    /**
     * Whether or not this task list is, or has become as result of
     * task decomposition, an empty one.
     *
    ...
    /**
     * This function replaces (i.e., decomposes) a given task atom with
     * a task list.
     *
     * @param tasksIn the decomposition to replace the task atom.
     */
     ...
     * This number is used to return bindings of the appropriate size.
     *
     * @param varCountIn the number of variables for this task list.
     */
    ...
    /**
     * This function undoes the decomposition of a task atom to a task
     * list in case of a backtrack over the decision to decompose the task.
     */
    public void undo() {
