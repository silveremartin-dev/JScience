...
public class State {
     ...
     * The array is indexed first by the head of the predicates each axiom can
     * prove and second by the axioms themselves.
     */
...
/**
     ...
     *
     * @param size     the number of possible heads of predicates (i.e., the number
     *                 of constant symbols that can come first in a predicate).
     * @param axiomsIn the axioms in the domain description as a
     *                 two-dimensional array. The array is indexed first by the head of
     *                 the predicates each axiom can prove and second by the axioms
     *                 themselves.
     */
     ...
     *
     * @param p the predicate to be protected.
     *
     ...
     *
     * @param p the predicate to be deleted.
     *
     ...
     *
     * @param p the predicate to be unprotected.
     *
     ...
     *
     * @param head the index of the constant symbol that is the head of the
     *        predicate (i.e., that comes first in the predicate).
     *
     * @return the iterator to be used to find the satisfiers for this
     *         predicate.
     */
    ...
    /**
     * This function returns the bindings that can satisfy a given
     * precondition one-by-one.
     *
     * @param p the predicate to be satisfied.
     * @param me the iterator that keeps track of where we are with the
     *        satisfiers so that the next time this function is called, we can
     *        take off where we stopped last time.
     *
        ...
        while (true) {
            ...
            for (; me.index < me.ax.getBranchSize(); me.index++) {
                ...
                while ((nextB = me.pre.nextBinding()) != null) {
                    ...
                    Predicate groundAxiomHead = me.ax.getHead()
                                                     .applySubstitution(nextB);
                ...
                //-- According to the semantics of the axiom branches in Planner, second
    ...
    /**
     * This function is used, in case of a backtrack, to undo the
     * changes that were made to the current state of the world because of the
     * backtracked decision.
     *
     * @param delAdd a 4-member array of type <code>Vector</code>. These four
