...
public class LogicalExpressionConjunction extends LogicalExpression {
    /**
     * The number of objects instantiated from this class before this
     * object was instantiated. Used to make the name of the precondition
     * class that implements this conjunction unique.
     */
    private int cnt;

    /**
     * An array of logical expressions the conjunction of which is
...
/**
     ...
     *
     * @param leIn a <code>Vector</code> of logical expressions the conjunction
     ...
     * object of which can be used at run time to represent the conjuncts of
     * this conjunction, and the conjunction itself.
     *
     * @return DOCUMENT ME!
     */
    public String getInitCode() {
        ...
        //-- conjunct.
        s += ("{" + endl + "\tPrecondition[] p;" + endl + "\tTerm[][] b;" +
        endl);
        ...
        s += ("\t{" + endl + "\t\tp = new Precondition[" + le.length + "];" +
        endl);
        ...
        s += ("\t}" + endl + endl + "\tprotected Term[] nextBindingHelper()" +
        endl);
        s += ("\t{" + endl);
    ...
    private String getInitCodeNext() {
        ...
        //-- null.
        s = "\t\tif (b[0] == null)" + endl + "\t\t\treturn null;" + endl +
            endl;
        ...
        for (i = le.length - 1, tabs = "\t\t"; i > 0; i--, tabs += "\t") {
            ...
            s += (tabs + "while (b[" + i + "] == null)" + endl + tabs + "{" +
            endl);
        }
        ...
        s += (tabs + "if (b[0] == null)" + endl + tabs + "\treturn null;" +
        endl);
        ...
        for (i = 2; i < le.length;
                i++, tabs = tabs.substring(0, tabs.length() - 1)) {
        ...
        //-- conjunct.
        return s + "\t\t}" + endl + endl + "\t\treturn Term.merge(b, " +
        le.length + ");" + endl;
    }
    ...
    private String getInitCodeReset() {
        ...
        for (j = 1; j < (le.length - 1); j++) {
            ...
            s += (tabs + "if (b[0] == null)" + endl + tabs + "\treturn;" +
            endl);

            //-- Try the second outer most conjunct.
            s += (tabs + "p[1].reset();" + endl + tabs + "p[1].bind(b[0]);" +
            endl);
            s += (tabs + "b[1] = p[1].nextBinding();" + endl);
            ...
            for (i = 2; i <= j;
                    i++, tabs = tabs.substring(0, tabs.length() - 1)) {
                s += (tabs + "}" + endl + tabs + "p[" + i + "].reset();" +
                endl);
                s += (tabs + "p[" + i + "].bind(Term.merge(b, " + i + "));" +
                endl);
                s += (tabs + "b[" + i + "] = p[" + i + "].nextBinding();" +
                endl);
            }
    ...
    /**
     * To propagate the variable count to all the logical expressions
     * the conjunction of which this object represents.
     *
    ...
    /**
     * This function produces the Java code to create an object of the
     * class that was implemented to represent this conjunction at run time.
     *
     * @return DOCUMENT ME!
     */
