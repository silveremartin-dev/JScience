...
public class AstronomicalPosition {
    ...
    public AstronomicalPosition(double l, double b) throws TransformationException {
	this (l, b, "J2000");
    }
    
    /** Define a position object used a specified frame */
    public AstronomicalPosition(double l, double b, String frame) throws TransformationException {
	...
	if (frame == null || frame.toUpperCase().equals("J2000")) {
	    ...
	    double[] unit         = CoordinatesUtils.unit(Math.toRadians(l), Math.toRadians(b));
	    ...
	    double[] j2000C    = CoordinatesUtils.coord(j2000Unit);
	...
	if (frame == null || frame.toUpperCase().equals("J2000")) {
	    ...
	    CoordinateSystem csys = CoordinateSystem.factory(frame);
	    double[] unit         = CoordinatesUtils.unit(Math.toRadians(coords[0]), Math.toRadians(coords[1]));
	    Converter conv        = new Converter();
	    ...
	    double[] xCoords = CoordinatesUtils.coord(xUnit);
	...
	AstronomicalPosition p   = new AstronomicalPosition(l, b, args[2]);
