/**
 * \ AtlasRef AtlasRef  is a class that figures out which page in a number of
 * atlases given an R.A. and Dec. It also supports the Rukl lunar atlas given
 * a lunar latitude and longitude. Based in part on C code from project pluto
 * (www.projectpluto.com) \
 */
package org.jscience.astronomy.catalogs;

/**
 * CatalogsUtils is a class that figures out which page in a number of atlases best
 * show the given RA and Dec.
 * <p/>
 * <p/>
 * It also supports the Rukl lunar atlas given a lunar latitude and longitude.
 * </p>
 */
public class CatalogsUtils {
    /**
    ...
    public static int millenniumAtlasPage(double ra, double dec) {
        ...
        if (dec >= 87.) { /* polar cap pages */
            ...
            final int[] per_zone = {
                    2, 4, 8, 10, 12, 14, 16, 20, 20, 22, 22, 24, 24, 24, 24, 24,
                    24, 24, 24, 24, 22, 22, 20, 20, 16, 14, 12, 10, 8, 4, 2
            };

            page = (int) ((remains * (double) per_zone[zone]) / 8.) + 1 +
                    (gore * 516);
    ...
    public static int uranometriaPage(double ra, double dec, boolean fix472) {
        final int[] decLimits = {
                -900, -845, -725, -610, -500, -390, -280, -170, -55, 55, 170,
        ...
        for (divide = 0; (double) decLimits[divide + 1] < (dec * 10.);
             divide++)
