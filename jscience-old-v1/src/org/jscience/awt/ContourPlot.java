...
/**
    ...
    public Dimension getMinimumSize() {
        ...
        return new Dimension(largeurMax + (2 * Contourx),
            data.length + (2 * Contoury));
    }
    ...
    private void rescale() {
        ...
        for (int k = 0; k < data.length; k++) {
            ...
            if ((data[k].length * deltax[k]) > largeurMax) {
                largeurMax = data[k].length * deltax[k];
            }
        }
    ...
    /**
     * Set the color for the graph. Note that the box around the graph
     * has this same color.
     *
    ...
    protected void offscreenPaint(Graphics g) {
        ...
        for (int k = 0; k < data.length; k++) {
            for (int l = 0; l < data[k].length; l++) {
                ...
                g.fillRect((l * deltax[k]) + Contourx, (k * deltay) + Contoury,
                    ((l + 1) * deltax[k]) + Contourx,
                    ((k + 1) * deltay) + Contoury);
            }
        }
    }
