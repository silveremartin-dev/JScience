/**
 * The LX200 class encapsulates an LX200 telescope. Copyright (C) 1999-2001
 * Mark Hale
 */
...
/**
    ...
    public LX200(String port) {
        try {
            ...
            System.err.println("Port is in use by another process: " +
                e.getMessage());
            e.printStackTrace();
     ...
     * Sets high precision.
     *
     * @param setHigh DOCUMENT ME!
     *
     * @throws IOException DOCUMENT ME!
     */
    public synchronized void setHighPrecision(boolean setHigh)
        throws IOException {
     ...
     * Sets long format.
     *
     * @param setLong DOCUMENT ME!
     *
     * @throws IOException DOCUMENT ME!
     */
    public synchronized void setLongFormat(boolean setLong)
        throws IOException {
     ...
     * @throws IOException DOCUMENT ME!
     */
    public synchronized void startFocus(int direction)
        throws IOException {
        switch (direction) {
        case FOCUS_IN:
            sendCmd("#:F+#");
     ...
     * @throws IOException DOCUMENT ME!
     */
    public synchronized boolean setObjectCoords(String ra, String dec)
        throws IOException {
        boolean rc;
        sendCmd("#:Sr" + ra + "#");
        rc = readBoolean();
     ...
     *
     * @return 0 if slew is possible, 1 if object is below the horizon, 2 if
     *         object is below the higher.
     *
     * @throws IOException DOCUMENT ME!
     */
     ...
     * @throws IOException DOCUMENT ME!
     */
    public synchronized boolean checkPosition(float ra, float dec)
        throws IOException {
        final float raError = raToFloat(getRA()) - ra;
        final float decError = decToFloat(getDec()) - dec;

        if ((Math.abs(raError) > (1.0 / (15.0 * 30.0))) ||
                (Math.abs(decError) > (1.0 / 30.0))) {
            return false;
        } else {
            return true;
        }
     ...
     * @throws IOException DOCUMENT ME!
     */
    public synchronized boolean setLocalTime(String time)
        throws IOException {
        sendCmd("#:SL" + time + "#");
