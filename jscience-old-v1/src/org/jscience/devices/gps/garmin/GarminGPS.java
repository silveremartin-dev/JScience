...
import org.jscience.devices.gps.GPS;
...
public class GarminGPS extends GPS implements Runnable {
    ...
    public GarminGPS(BufferedInputStream i, BufferedOutputStream o) {
        ...
        try {
            ...
            output.write(GarminPacket.createBasicPacket(
                    GarminPacket.Pid_Product_Rqst, new int[]{}));
        } catch (IOException e) {
        }
    }
    ...
    public void run() {
        ...
        while (active) {
            ...
            try {
                output.write(GarminPacket.createBasicPacket(
                        GarminPacket.Pid_Ack_Byte, new int[]{pack.getID(), 0}));
            } catch (IOException e) {
                active = false;
            }
    ...
    /**
     * This method is used to identify the type of packet received, and
    ...
    public void requestPosition() {
        try {
            output.write(GarminPacket.createCommandPacket(
                    GarminPacket.Cmnd_Transfer_Posn));
        } catch (IOException e) {
        }
    }
    ...
    public void requestTime() {
        try {
            output.write(GarminPacket.createCommandPacket(
                    GarminPacket.Cmnd_Transfer_Time));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void requestWaypoints() {
        try {
            output.write(GarminPacket.createCommandPacket(
                    GarminPacket.Cmnd_Transfer_Wpt));
        } catch (IOException e) {
        }
    }
    ...
    public void requestTracks() {
        try {
            output.write(GarminPacket.createCommandPacket(
                    GarminPacket.Cmnd_Transfer_Trk));
        } catch (IOException e) {
        }
    }
    ...
    public void requestLaps() {
        try {
            output.write(GarminPacket.createCommandPacket(
                    GarminPacket.Cmnd_Transfer_Laps));
        } catch (IOException e) {
        }
    }
     ...
     * > The data will be that which is accessible through the
    ...
    public void setAutoTransmit(boolean t) {
        if (t) {
            try {
                output.write(GarminPacket.createCommandPacket(
                        GarminPacket.Cmnd_Start_Pvt_Data));
            } catch (IOException e) {
            }
        } else {
            try {
                output.write(GarminPacket.createCommandPacket(
                        GarminPacket.Cmnd_Stop_Pvt_Data));
            } catch (IOException e) {
            }
        }
    }
     ...
     * off. If b is set to false, the GPS will remain turned on.
     *
     * @param b DOCUMENT ME!
     */
    public void shutdown(boolean b) {
        if (b) {
            try {
                output.write(GarminPacket.createCommandPacket(
                        GarminPacket.Cmnd_Turn_Off_Pwr));
            } catch (IOException e) {
            }
        }
