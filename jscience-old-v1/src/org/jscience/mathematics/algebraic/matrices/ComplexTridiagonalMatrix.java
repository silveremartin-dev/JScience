...
/**
 * The ComplexTridiagonalMatrix class provides an object for encapsulating
 * tridiagonal matrices containing complex numbers. Uses compressed diagonal
 * storage.
 *
 * @author Mark Hale
 * @version 2.2
 */
public class ComplexTridiagonalMatrix extends AbstractComplexSquareMatrix
    implements Cloneable, Serializable, TridiagonalMatrix {
    /** Tridiagonal data. */
...
/**
     ...
     */
    public ComplexTridiagonalMatrix(final ComplexTridiagonalMatrix mat) {
        this(mat.numRows());
        System.arraycopy(mat.ldiagRe, 0, ldiagRe, 0, mat.ldiagRe.length);
        System.arraycopy(mat.ldiagIm, 0, ldiagIm, 0, mat.ldiagIm.length);
        System.arraycopy(mat.diagRe, 0, diagRe, 0, mat.diagRe.length);
        System.arraycopy(mat.diagIm, 0, diagIm, 0, mat.diagIm.length);
        System.arraycopy(mat.udiagRe, 0, udiagRe, 0, mat.udiagRe.length);
        System.arraycopy(mat.udiagIm, 0, udiagIm, 0, mat.udiagIm.length);
    }
    ...
    public boolean equals(AbstractComplexMatrix m, double tol) {
        if (m instanceof TridiagonalMatrix) {
            ...
            (udeltaRe * udeltaRe) + (udeltaIm * udeltaIm));
            ...
            for (; i < (numRows() - 1); i++) {
                ...
                sumSqr += ((ldeltaRe * ldeltaRe) + (ldeltaIm * ldeltaIm) +
    ...
    public String toString() {
        ...
        for (int i = 0; i < numRows(); i++) {
            for (int j = 0; j < numColumns(); j++) {
                buf.append(getPrimitiveElement(i, j).toString());
                buf.append(' ');
            }
    ...
    public void setAllElements(final Complex r) {
        if (r.equals(Complex.ZERO)) {
            ...
            throw new IllegalArgumentException(
                "You can't set all elements of a tridiagonal matrix unless to zero.");
        }
    }
    ...
    public boolean isSymmetric() {
        if (numElements() > 1) {
            if ((ldiagRe[1] != udiagRe[0]) || (ldiagIm[1] != udiagIm[0])) {
                return false;
            }

            for (int i = 1; i < (numRows() - 1); i++) {
                if ((ldiagRe[i + 1] != udiagRe[i]) ||
                        (ldiagIm[i + 1] != udiagIm[i])) {
                    return false;
                }
            }
    ...
    public double infNorm() {
        if (numElements() > 0) {
            double result = Math.sqrt(((diagRe[0] * diagRe[0]) +
                    (diagIm[0] * diagIm[0]))) +
                Math.sqrt(((udiagRe[0] * udiagRe[0]) +
                    (udiagIm[0] * udiagIm[0])));
            double tmpResult;
            int i = 1;

            for (; i < (numRows() - 1); i++) {
                tmpResult = Math.sqrt(((ldiagRe[i] * ldiagRe[i]) +
                        (ldiagIm[i] * ldiagIm[i]))) +
                    Math.sqrt(((diagRe[i] * diagRe[i]) +
                        (diagIm[i] * diagIm[i]))) +
                    Math.sqrt(((udiagRe[i] * udiagRe[i]) +
                        (udiagIm[i] * udiagIm[i])));
            ...
            tmpResult = Math.sqrt(((ldiagRe[i] * ldiagRe[i]) +
                    (ldiagIm[i] * ldiagIm[i]))) +
                Math.sqrt(((diagRe[i] * diagRe[i]) + (diagIm[i] * diagIm[i])));
    ...
    public double frobeniusNorm() {
        if (numElements() > 0) {
            ...
            for (; i < (numRows() - 1); i++) {
                result += ((ldiagRe[i] * ldiagRe[i]) +
                (ldiagIm[i] * ldiagIm[i]) + (diagRe[i] * diagRe[i]) +
                (diagIm[i] * diagIm[i]) + (udiagRe[i] * udiagRe[i]) +
                (udiagIm[i] * udiagIm[i]));
            }
     ...
     * @return DOCUMENT ME!
     *
     * @throws MaximumIterationsExceededException If it takes more than 50
     *         iterations to determine an eigenvalue.
     * @throws ArithmeticException DOCUMENT ME!
     */
    public double operatorNorm() throws MaximumIterationsExceededException {
        if (numElements() > 0) {
            return Math.sqrt(ArrayMathUtils.max(
                    LinearMathUtils.eigenvalueSolveHermitian(
                        (ComplexTridiagonalMatrix) (this.hermitianAdjoint()
                                                        .multiply(this)))));
        } else {
            throw new ArithmeticException(
                "The operator norm of a zero dimension matrix is undefined.");
        }
    }
    ...
    public AbstractComplexSquareMatrix add(final AbstractComplexSquareMatrix m) {
        ...
        if ((numRows() == m.numRows()) && (numColumns() == m.numColumns())) {
            ...
            for (int i = 0; i < numRows(); i++) {
                Complex elem = getPrimitiveElement(i, 0)
                                   .add(m.getPrimitiveElement(i, 0));
                arrayRe[i][0] = elem.real();
                arrayIm[i][0] = elem.imag();

                for (int j = 1; j < numColumns(); j++) {
                    elem = getPrimitiveElement(i, j)
    ...
    public AbstractComplexSquareMatrix subtract(
        ...
        if ((numRows() == m.numRows()) && (numColumns() == m.numColumns())) {
            ...
            for (int i = 0; i < numRows(); i++) {
                elem = getPrimitiveElement(i, 0)
                           .subtract(m.getPrimitiveElement(i, 0));
                arrayRe[i][0] = elem.real();
                arrayIm[i][0] = elem.imag();
    ...
    public AbstractComplexVector multiply(final AbstractComplexVector v) {
        ...
        if (mRow == v.getDimension()) {
            ...
            arrayIm[0] = ((diagIm[0] * comp.real()) +
                (diagRe[0] * comp.imag()));
            comp = v.getPrimitiveElement(1);
            arrayRe[0] += ((udiagRe[0] * comp.real()) -
            (udiagIm[0] * comp.imag()));
            arrayIm[0] += ((udiagIm[0] * comp.real()) +
            (udiagRe[0] * comp.imag()));
            mRow--;

            for (int i = 1; i < mRow; i++) {
                comp = v.getPrimitiveElement(i - 1);
                arrayRe[i] = ((ldiagRe[i] * comp.real()) -
                    (ldiagIm[i] * comp.imag()));
                arrayIm[i] = ((ldiagIm[i] * comp.real()) +
                    (ldiagRe[i] * comp.imag()));
                ...
                (diagRe[i] * comp.imag()));
                comp = v.getPrimitiveElement(i + 1);
                arrayRe[i] += ((udiagRe[i] * comp.real()) -
                (udiagIm[i] * comp.imag()));
                arrayIm[i] += ((udiagIm[i] * comp.real()) +
                (udiagRe[i] * comp.imag()));
            }

            comp = v.getPrimitiveElement(mRow - 1);
            arrayRe[mRow] = ((ldiagRe[mRow] * comp.real()) -
                (ldiagIm[mRow] * comp.imag()));
            arrayIm[mRow] = ((ldiagIm[mRow] * comp.real()) +
                (ldiagRe[mRow] * comp.imag()));
            comp = v.getPrimitiveElement(mRow);
            arrayRe[mRow] += ((diagRe[mRow] * comp.real()) -
            (diagIm[mRow] * comp.imag()));
            arrayIm[mRow] += ((diagIm[mRow] * comp.real()) +
            (diagRe[mRow] * comp.imag()));
        ...
        if (numColumns() == m.numRows()) {
            ...
            for (int j = 0; j < numRows(); j++) {
                for (int k = 0; k < m.numColumns(); k++) {
                    Complex elem = getPrimitiveElement(j, 0)
                                       .multiply(m.getPrimitiveElement(0, k));
                    arrayRe[j][k] = elem.real();
                    arrayIm[j][k] = elem.imag();

                    for (int n = 1; n < numColumns(); n++) {
                        elem = getPrimitiveElement(j, n)
                                   .multiply(m.getPrimitiveElement(n, k));
                        arrayRe[j][k] += elem.real();
                        arrayIm[j][k] += elem.imag();
                    }
                }
    ...
    public ComplexSquareMatrix multiply(final ComplexSquareMatrix m) {
        if (numColumns() == m.numRows()) {
            ...
            for (int j = 0; j < numRows(); j++) {
                for (int k = 0; k < m.numColumns(); k++) {
                    Complex elem = getPrimitiveElement(j, 0);
                    double real = ((m.matrixRe[0][k] * elem.real()) -
                        (m.matrixIm[0][k] * elem.imag()));
                    double imag = ((m.matrixIm[0][k] * elem.real()) +
                        (m.matrixRe[0][k] * elem.imag()));

                    for (int n = 1; n < numColumns(); n++) {
                        elem = getPrimitiveElement(j, n);
                        real += ((m.matrixRe[n][k] * elem.real()) -
                        (m.matrixIm[n][k] * elem.imag()));
                        imag += ((m.matrixIm[n][k] * elem.real()) +
                        (m.matrixRe[n][k] * elem.imag()));
                    }
    ...
    public ComplexSquareMatrix multiply(final ComplexTridiagonalMatrix m) {
        ...
        if (numColumns() == m.numRows()) {
            ...
            arrayRe[0][0] = ((diagRe[0] * m.diagRe[0]) -
                (diagIm[0] * m.diagIm[0])) +
                ((udiagRe[0] * m.ldiagRe[1]) - (udiagIm[0] * m.ldiagIm[1]));
            arrayIm[0][0] = ((diagIm[0] * m.diagRe[0]) +
                (diagRe[0] * m.diagIm[0])) +
                ((udiagIm[0] * m.ldiagRe[1]) + (udiagRe[0] * m.ldiagIm[1]));
            arrayRe[0][1] = ((diagRe[0] * m.udiagRe[0]) -
                (diagIm[0] * m.udiagIm[0])) +
                ((udiagRe[0] * m.diagRe[1]) - (udiagIm[0] * m.diagIm[1]));
            arrayIm[0][1] = ((diagIm[0] * m.udiagRe[0]) +
                (diagRe[0] * m.udiagIm[0])) +
                ((udiagIm[0] * m.diagRe[1]) + (udiagRe[0] * m.diagIm[1]));
            arrayRe[0][2] = ((udiagRe[0] * m.udiagRe[1]) -
                (udiagIm[0] * m.udiagIm[1]));
            arrayIm[0][2] = ((udiagIm[0] * m.udiagRe[1]) +
                (udiagRe[0] * m.udiagIm[1]));

            if (mRow > 3) {
                arrayRe[1][0] = ((ldiagRe[1] * m.diagRe[0]) -
                    (ldiagIm[1] * m.diagIm[0])) +
                    ((diagRe[1] * m.ldiagRe[1]) - (diagIm[1] * m.ldiagIm[1]));
                arrayIm[1][0] = ((ldiagIm[1] * m.diagRe[0]) +
                    (ldiagRe[1] * m.diagIm[0])) +
                    ((diagIm[1] * m.ldiagRe[1]) + (diagRe[1] * m.ldiagIm[1]));
                arrayRe[1][1] = ((ldiagRe[1] * m.udiagRe[0]) -
                    (ldiagIm[1] * m.udiagIm[0])) +
                    ((diagRe[1] * m.diagRe[1]) - (diagIm[1] * m.diagIm[1])) +
                    ((udiagRe[1] * m.ldiagRe[2]) - (udiagIm[1] * m.ldiagIm[2]));
                arrayIm[1][1] = ((ldiagIm[1] * m.udiagRe[0]) +
                    (ldiagRe[1] * m.udiagIm[0])) +
                    ((diagIm[1] * m.diagRe[1]) + (diagRe[1] * m.diagIm[1])) +
                    ((udiagIm[1] * m.ldiagRe[2]) + (udiagRe[1] * m.ldiagIm[2]));
                arrayRe[1][2] = ((diagRe[1] * m.udiagRe[1]) -
                    (diagIm[1] * m.udiagIm[1])) +
                    ((udiagRe[1] * m.diagRe[2]) - (udiagIm[1] * m.diagIm[2]));
                arrayIm[1][2] = ((diagIm[1] * m.udiagRe[1]) +
                    (diagRe[1] * m.udiagIm[1])) +
                    ((udiagIm[1] * m.diagRe[2]) + (udiagRe[1] * m.diagIm[2]));
                arrayRe[1][3] = ((udiagRe[1] * m.udiagRe[2]) -
                    (udiagIm[1] * m.udiagIm[2]));
                arrayIm[1][3] = ((udiagIm[1] * m.udiagRe[2]) +
                    (udiagRe[1] * m.udiagIm[2]));
            }
            ...
            if (mRow == 3) {
                ...
                for (int i = 2; i < (mRow - 2); i++) {
                    arrayRe[i][i - 2] = ((ldiagRe[i] * m.ldiagRe[i - 1]) -
                        (ldiagIm[i] * m.ldiagIm[i - 1]));
                    arrayIm[i][i - 2] = ((ldiagIm[i] * m.ldiagRe[i - 1]) +
                        (ldiagRe[i] * m.ldiagIm[i - 1]));
                    arrayRe[i][i - 1] = ((ldiagRe[i] * m.diagRe[i - 1]) -
                        (ldiagIm[i] * m.diagIm[i - 1])) +
                        ((diagRe[i] * m.ldiagRe[i]) -
                        (diagIm[i] * m.ldiagIm[i]));
                    arrayIm[i][i - 1] = ((ldiagIm[i] * m.diagRe[i - 1]) +
                        (ldiagRe[i] * m.diagIm[i - 1])) +
                        ((diagIm[i] * m.ldiagRe[i]) +
                        (diagRe[i] * m.ldiagIm[i]));
                    arrayRe[i][i] = ((ldiagRe[i] * m.udiagRe[i - 1]) -
                        (ldiagIm[i] * m.udiagIm[i - 1])) +
                        ((diagRe[i] * m.diagRe[i]) - (diagIm[i] * m.diagIm[i])) +
                        ((udiagRe[i] * m.ldiagRe[i + 1]) -
                        (udiagIm[i] * m.ldiagIm[i + 1]));
                    arrayIm[i][i] = ((ldiagIm[i] * m.udiagRe[i - 1]) +
                        (ldiagRe[i] * m.udiagIm[i - 1])) +
                        ((diagIm[i] * m.diagRe[i]) + (diagRe[i] * m.diagIm[i])) +
                        ((udiagIm[i] * m.ldiagRe[i + 1]) +
                        (udiagRe[i] * m.ldiagIm[i + 1]));
                    arrayRe[i][i + 1] = ((diagRe[i] * m.udiagRe[i]) -
                        (diagIm[i] * m.udiagIm[i])) +
                        ((udiagRe[i] * m.diagRe[i + 1]) -
                        (udiagIm[i] * m.diagIm[i + 1]));
                    arrayIm[i][i + 1] = ((diagIm[i] * m.udiagRe[i]) +
                        (diagRe[i] * m.udiagIm[i])) +
                        ((udiagIm[i] * m.diagRe[i + 1]) +
                        (udiagRe[i] * m.diagIm[i + 1]));
                    arrayRe[i][i + 2] = ((udiagRe[i] * m.udiagRe[i + 1]) -
                        (udiagIm[i] * m.udiagIm[i + 1]));
                    arrayIm[i][i + 2] = ((udiagIm[i] * m.udiagRe[i + 1]) +
                        (udiagRe[i] * m.udiagIm[i + 1]));
                }
            }

            if (mRow > 3) {
                arrayRe[mRow - 2][mRow - 4] = ((ldiagRe[mRow - 2] * m.ldiagRe[mRow -
                    3]) - (ldiagIm[mRow - 2] * m.ldiagIm[mRow - 3]));
                arrayIm[mRow - 2][mRow - 4] = ((ldiagIm[mRow - 2] * m.ldiagRe[mRow -
                    3]) + (ldiagRe[mRow - 2] * m.ldiagIm[mRow - 3]));
                arrayRe[mRow - 2][mRow - 3] = ((ldiagRe[mRow - 2] * m.diagRe[mRow -
                    3]) - (ldiagIm[mRow - 2] * m.diagIm[mRow - 3])) +
                    ((diagRe[mRow - 2] * m.ldiagRe[mRow - 2]) -
                    (diagIm[mRow - 2] * m.ldiagIm[mRow - 2]));
                arrayIm[mRow - 2][mRow - 3] = ((ldiagIm[mRow - 2] * m.diagRe[mRow -
                    3]) + (ldiagRe[mRow - 2] * m.diagIm[mRow - 3])) +
                    ((diagIm[mRow - 2] * m.ldiagRe[mRow - 2]) +
                    (diagRe[mRow - 2] * m.ldiagIm[mRow - 2]));
                arrayRe[mRow - 2][mRow - 2] = ((ldiagRe[mRow - 2] * m.udiagRe[mRow -
                    3]) - (ldiagIm[mRow - 2] * m.udiagIm[mRow - 3])) +
                    ((diagRe[mRow - 2] * m.diagRe[mRow - 2]) -
                    (diagIm[mRow - 2] * m.diagIm[mRow - 2])) +
                    ((udiagRe[mRow - 2] * m.ldiagRe[mRow - 1]) -
                    (udiagIm[mRow - 2] * m.ldiagIm[mRow - 1]));
                arrayIm[mRow - 2][mRow - 2] = ((ldiagIm[mRow - 2] * m.udiagRe[mRow -
                    3]) + (ldiagRe[mRow - 2] * m.udiagIm[mRow - 3])) +
                    ((diagIm[mRow - 2] * m.diagRe[mRow - 2]) +
                    (diagRe[mRow - 2] * m.diagIm[mRow - 2])) +
                    ((udiagIm[mRow - 2] * m.ldiagRe[mRow - 1]) +
                    (udiagRe[mRow - 2] * m.ldiagIm[mRow - 1]));
                arrayRe[mRow - 2][mRow - 1] = ((diagRe[mRow - 2] * m.udiagRe[mRow -
                    2]) - (diagIm[mRow - 2] * m.udiagIm[mRow - 2])) +
                    ((udiagRe[mRow - 2] * m.diagRe[mRow - 1]) -
                    (udiagIm[mRow - 2] * m.diagIm[mRow - 1]));
                arrayIm[mRow - 2][mRow - 1] = ((diagIm[mRow - 2] * m.udiagRe[mRow -
                    2]) + (diagRe[mRow - 2] * m.udiagIm[mRow - 2])) +
                    ((udiagIm[mRow - 2] * m.diagRe[mRow - 1]) +
                    (udiagRe[mRow - 2] * m.diagIm[mRow - 1]));
            }

            mRow--;
            arrayRe[mRow][mRow - 2] = ((ldiagRe[mRow] * m.ldiagRe[mRow - 1]) -
                (ldiagIm[mRow] * m.ldiagIm[mRow - 1]));
            arrayIm[mRow][mRow - 2] = ((ldiagIm[mRow] * m.ldiagRe[mRow - 1]) +
                (ldiagRe[mRow] * m.ldiagIm[mRow - 1]));
            arrayRe[mRow][mRow - 1] = ((ldiagRe[mRow] * m.diagRe[mRow - 1]) -
                (ldiagIm[mRow] * m.diagIm[mRow - 1])) +
                ((diagRe[mRow] * m.ldiagRe[mRow]) -
                (diagIm[mRow] * m.ldiagIm[mRow]));
            arrayIm[mRow][mRow - 1] = ((ldiagIm[mRow] * m.diagRe[mRow - 1]) +
                (ldiagRe[mRow] * m.diagIm[mRow - 1])) +
                ((diagIm[mRow] * m.ldiagRe[mRow]) +
                (diagRe[mRow] * m.ldiagIm[mRow]));
            arrayRe[mRow][mRow] = ((ldiagRe[mRow] * m.udiagRe[mRow - 1]) -
                (ldiagIm[mRow] * m.udiagIm[mRow - 1])) +
                ((diagRe[mRow] * m.diagRe[mRow]) -
                (diagIm[mRow] * m.diagIm[mRow]));
            arrayIm[mRow][mRow] = ((ldiagIm[mRow] * m.udiagRe[mRow - 1]) +
                (ldiagRe[mRow] * m.udiagIm[mRow - 1])) +
                ((diagIm[mRow] * m.diagRe[mRow]) +
     ...
     * @return DOCUMENT ME!
     */
    private ComplexSquareMatrix multiplyTridiagonal(
        final AbstractComplexSquareMatrix m) {
        int mRow = numRows();
        ...
        if (numColumns() == m.numRows()) {
            ...
            Complex elem2;
            Complex elem3;
            elem1 = m.getPrimitiveElement(0, 0);
            elem2 = m.getPrimitiveElement(1, 0);
            arrayRe[0][0] = ((diagRe[0] * elem1.real()) -
                (diagIm[0] * elem1.imag())) +
                ((udiagRe[0] * elem2.real()) - (udiagIm[0] * elem2.imag()));
            arrayIm[0][0] = ((diagIm[0] * elem1.real()) +
                (diagRe[0] * elem1.imag())) +
                ((udiagIm[0] * elem2.real()) + (udiagRe[0] * elem2.imag()));
            elem1 = m.getPrimitiveElement(0, 1);
            elem2 = m.getPrimitiveElement(1, 1);
            arrayRe[0][1] = ((diagRe[0] * elem1.real()) -
                (diagIm[0] * elem1.imag())) +
                ((udiagRe[0] * elem2.real()) - (udiagIm[0] * elem2.imag()));
            arrayIm[0][1] = ((diagIm[0] * elem1.real()) +
                (diagRe[0] * elem1.imag())) +
                ((udiagIm[0] * elem2.real()) + (udiagRe[0] * elem2.imag()));
            elem1 = m.getPrimitiveElement(1, 2);
            arrayRe[0][2] = ((udiagRe[0] * elem1.real()) -
                (udiagIm[0] * elem1.imag()));
            arrayIm[0][2] = ((udiagIm[0] * elem1.real()) +
                (udiagRe[0] * elem1.imag()));

            if (mRow > 3) {
                elem1 = m.getPrimitiveElement(0, 0);
                elem2 = m.getPrimitiveElement(1, 0);
                arrayRe[1][0] = ((ldiagRe[1] * elem1.real()) -
                    (ldiagIm[1] * elem1.imag())) +
                    ((diagRe[1] * elem2.real()) - (diagIm[1] * elem2.imag()));
                arrayIm[1][0] = ((ldiagIm[1] * elem1.real()) +
                    (ldiagRe[1] * elem1.imag())) +
                    ((diagIm[1] * elem2.real()) + (diagRe[1] * elem2.imag()));
                elem1 = m.getPrimitiveElement(0, 1);
                elem2 = m.getPrimitiveElement(1, 1);
                elem3 = m.getPrimitiveElement(2, 1);
                arrayRe[1][1] = ((ldiagRe[1] * elem1.real()) -
                    (ldiagIm[1] * elem1.imag())) +
                    ((diagRe[1] * elem2.real()) - (diagIm[1] * elem2.imag())) +
                    ((udiagRe[1] * elem3.real()) - (udiagIm[1] * elem3.imag()));
                arrayIm[1][1] = ((ldiagIm[1] * elem1.real()) +
                    (ldiagRe[1] * elem1.imag())) +
                    ((diagIm[1] * elem2.real()) + (diagRe[1] * elem2.imag())) +
                    ((udiagIm[1] * elem3.real()) + (udiagRe[1] * elem3.imag()));
                elem1 = m.getPrimitiveElement(1, 2);
                elem2 = m.getPrimitiveElement(2, 2);
                arrayRe[1][2] = ((diagRe[1] * elem1.real()) -
                    (diagIm[1] * elem1.imag())) +
                    ((udiagRe[1] * elem2.real()) - (udiagIm[1] * elem2.imag()));
                arrayIm[1][2] = ((diagIm[1] * elem1.real()) +
                    (diagRe[1] * elem1.imag())) +
                    ((udiagIm[1] * elem2.real()) + (udiagRe[1] * elem2.imag()));
                elem1 = m.getPrimitiveElement(2, 3);
                arrayRe[1][3] = ((udiagRe[1] * elem1.real()) -
                    (udiagIm[1] * elem1.imag()));
                arrayIm[1][3] = ((udiagIm[1] * elem1.real()) +
                    (udiagRe[1] * elem1.imag()));
            }
            ...
            if (mRow == 3) {
                ...
                for (int i = 2; i < (mRow - 2); i++) {
                    elem1 = m.getPrimitiveElement(i - 1, i - 2);
                    arrayRe[i][i - 2] = ((ldiagRe[i] * elem1.real()) -
                        (ldiagIm[i] * elem1.imag()));
                    arrayIm[i][i - 2] = ((ldiagIm[i] * elem1.real()) +
                        (ldiagRe[i] * elem1.imag()));
                    elem1 = m.getPrimitiveElement(i - 1, i - 1);
                    elem2 = m.getPrimitiveElement(i, i - 1);
                    arrayRe[i][i - 1] = ((ldiagRe[i] * elem1.real()) -
                        (ldiagIm[i] * elem1.imag())) +
                        ((diagRe[i] * elem2.real()) -
                        (diagIm[i] * elem2.imag()));
                    arrayIm[i][i - 1] = ((ldiagIm[i] * elem1.real()) +
                        (ldiagRe[i] * elem1.imag())) +
                        ((diagIm[i] * elem2.real()) +
                        (diagRe[i] * elem2.imag()));
                    elem1 = m.getPrimitiveElement(i - 1, i);
                    elem2 = m.getPrimitiveElement(i, i);
                    elem3 = m.getPrimitiveElement(i + 1, i);
                    arrayRe[i][i] = ((ldiagRe[i] * elem1.real()) -
                        (ldiagIm[i] * elem1.imag())) +
                        ((diagRe[i] * elem2.real()) -
                        (diagIm[i] * elem2.imag())) +
                        ((udiagRe[i] * elem3.real()) -
                        (udiagIm[i] * elem3.imag()));
                    arrayIm[i][i] = ((ldiagIm[i] * elem1.real()) +
                        (ldiagRe[i] * elem1.imag())) +
                        ((diagIm[i] * elem2.real()) +
                        (diagRe[i] * elem2.imag())) +
                        ((udiagIm[i] * elem3.real()) +
                        (udiagRe[i] * elem3.imag()));
                    elem1 = m.getPrimitiveElement(i, i + 1);
                    elem2 = m.getPrimitiveElement(i + 1, i + 1);
                    arrayRe[i][i + 1] = ((diagRe[i] * elem1.real()) -
                        (diagIm[i] * elem1.imag())) +
                        ((udiagRe[i] * elem2.real()) -
                        (udiagIm[i] * elem2.imag()));
                    arrayIm[i][i + 1] = ((diagIm[i] * elem1.real()) +
                        (diagRe[i] * elem1.imag())) +
                        ((udiagIm[i] * elem2.real()) +
                        (udiagRe[i] * elem2.imag()));
                    elem1 = m.getPrimitiveElement(i + 1, i + 2);
                    arrayRe[i][i + 2] = ((udiagRe[i] * elem1.real()) -
                        (udiagIm[i] * elem1.imag()));
                    arrayIm[i][i + 2] = ((udiagIm[i] * elem1.real()) +
                        (udiagRe[i] * elem1.imag()));
                }
            }

            if (mRow > 3) {
                elem1 = m.getPrimitiveElement(mRow - 3, mRow - 4);
                arrayRe[mRow - 2][mRow - 4] = ((ldiagRe[mRow - 2] * elem1.real()) -
                    (ldiagIm[mRow - 2] * elem1.imag()));
                arrayIm[mRow - 2][mRow - 4] = ((ldiagIm[mRow - 2] * elem1.real()) +
                    (ldiagRe[mRow - 2] * elem1.imag()));
                elem1 = m.getPrimitiveElement(mRow - 3, mRow - 3);
                elem2 = m.getPrimitiveElement(mRow - 2, mRow - 3);
                arrayRe[mRow - 2][mRow - 3] = ((ldiagRe[mRow - 2] * elem1.real()) -
                    (ldiagIm[mRow - 2] * elem1.imag())) +
                    ((diagRe[mRow - 2] * elem2.real()) -
                    (diagIm[mRow - 2] * elem2.imag()));
                arrayIm[mRow - 2][mRow - 3] = ((ldiagIm[mRow - 2] * elem1.real()) +
                    (ldiagRe[mRow - 2] * elem1.imag())) +
                    ((diagIm[mRow - 2] * elem2.real()) +
                    (diagRe[mRow - 2] * elem2.imag()));
                elem1 = m.getPrimitiveElement(mRow - 3, mRow - 2);
                elem2 = m.getPrimitiveElement(mRow - 2, mRow - 2);
                elem3 = m.getPrimitiveElement(mRow - 1, mRow - 2);
                arrayRe[mRow - 2][mRow - 2] = ((ldiagRe[mRow - 2] * elem1.real()) -
                    (ldiagIm[mRow - 2] * elem1.imag())) +
                    ((diagRe[mRow - 2] * elem2.real()) -
                    (diagIm[mRow - 2] * elem2.imag())) +
                    ((udiagRe[mRow - 2] * elem3.real()) -
                    (udiagIm[mRow - 2] * elem3.imag()));
                arrayIm[mRow - 2][mRow - 2] = ((ldiagIm[mRow - 2] * elem1.real()) +
                    (ldiagRe[mRow - 2] * elem1.imag())) +
                    ((diagIm[mRow - 2] * elem2.real()) +
                    (diagRe[mRow - 2] * elem2.imag())) +
                    ((udiagIm[mRow - 2] * elem3.real()) +
                    (udiagRe[mRow - 2] * elem3.imag()));
                elem1 = m.getPrimitiveElement(mRow - 2, mRow - 1);
                elem2 = m.getPrimitiveElement(mRow - 1, mRow - 1);
                arrayRe[mRow - 2][mRow - 1] = ((diagRe[mRow - 2] * elem1.real()) -
                    (diagIm[mRow - 2] * elem1.imag())) +
                    ((udiagRe[mRow - 2] * elem2.real()) -
                    (udiagIm[mRow - 2] * elem2.imag()));
                arrayIm[mRow - 2][mRow - 1] = ((diagIm[mRow - 2] * elem1.real()) +
                    (diagRe[mRow - 2] * elem1.imag())) +
                    ((udiagIm[mRow - 2] * elem2.real()) +
                    (udiagRe[mRow - 2] * elem2.imag()));
            }

            mRow--;
            elem1 = m.getPrimitiveElement(mRow - 1, mRow - 2);
            arrayRe[mRow][mRow - 2] = ((ldiagRe[mRow] * elem1.real()) -
                (ldiagIm[mRow] * elem1.imag()));
            arrayIm[mRow][mRow - 2] = ((ldiagIm[mRow] * elem1.real()) +
                (ldiagRe[mRow] * elem1.imag()));
            elem1 = m.getPrimitiveElement(mRow - 1, mRow - 1);
            elem2 = m.getPrimitiveElement(mRow, mRow - 1);
            arrayRe[mRow][mRow - 1] = ((ldiagRe[mRow] * elem1.real()) -
                (ldiagIm[mRow] * elem1.imag())) +
                ((diagRe[mRow] * elem2.real()) - (diagIm[mRow] * elem2.imag()));
            arrayIm[mRow][mRow - 1] = ((ldiagIm[mRow] * elem1.real()) +
                (ldiagRe[mRow] * elem1.imag())) +
                ((diagIm[mRow] * elem2.real()) + (diagRe[mRow] * elem2.imag()));
            elem1 = m.getPrimitiveElement(mRow - 1, mRow);
            elem2 = m.getPrimitiveElement(mRow, mRow);
            arrayRe[mRow][mRow] = ((ldiagRe[mRow] * elem1.real()) -
                (ldiagIm[mRow] * elem1.imag())) +
                ((diagRe[mRow] * elem2.real()) - (diagIm[mRow] * elem2.imag()));
            arrayIm[mRow][mRow] = ((ldiagIm[mRow] * elem1.real()) +
                (ldiagRe[mRow] * elem1.imag())) +
                ((diagIm[mRow] * elem2.real()) + (diagRe[mRow] * elem2.imag()));
    ...
    public AbstractComplexSquareMatrix[] luDecompose(int[] pivot) {
        ...
        for (j = 0; j < N; j++) {
            for (i = 0; i < j; i++) {
                elem = getPrimitiveElement(pivot[i], j);
                tmpRe = elem.real();
                tmpIm = elem.imag();

                for (k = 0; k < i; k++) {
                    tmpRe -= ((arrayURe[i][k] * arrayURe[k][j]) -
                    (arrayUIm[i][k] * arrayUIm[k][j]));
                    tmpIm -= ((arrayUIm[i][k] * arrayURe[k][j]) +
                    (arrayURe[i][k] * arrayUIm[k][j]));
                }
    ...
    public AbstractComplexMatrix mapElements(final ComplexMapping f) {
        ...
        array[0][0] = f.map(new Complex(diagRe[0], diagIm[0]));
        array[0][1] = f.map(new Complex(udiagRe[0], udiagIm[0]));

        int n = numRows() - 1;

        for (int i = 1; i < n; i++) {
            array[i][i - 1] = f.map(new Complex(ldiagRe[i], ldiagIm[i]));
            array[i][i] = f.map(new Complex(diagRe[i], diagIm[i]));
            array[i][i + 1] = f.map(new Complex(udiagRe[i], udiagIm[i]));
        }

        array[n][n - 1] = f.map(new Complex(ldiagRe[n], ldiagIm[n]));
        array[n][n] = f.map(new Complex(diagRe[n], diagIm[n]));
     ...
     */
    public Object clone() {
        return new ComplexTridiagonalMatrix(this);
    }
    ...
    public Complex[][] toPrimitiveArray() {
        final Complex[][] array = new Complex[numRows()][numColumns()];
        array[0][0] = new Complex(diagRe[0], diagIm[0]);
        array[0][1] = new Complex(udiagRe[0], udiagIm[0]);

        int n = numRows() - 1;

        for (int i = 1; i < n; i++) {
            array[i][i - 1] = new Complex(ldiagRe[i], ldiagIm[i]);
            array[i][i] = new Complex(diagRe[i], diagIm[i]);
            array[i][i + 1] = new Complex(udiagRe[i], udiagIm[i]);
        }

        array[n][n - 1] = new Complex(ldiagRe[n], ldiagIm[n]);
        array[n][n] = new Complex(diagRe[n], diagIm[n]);
