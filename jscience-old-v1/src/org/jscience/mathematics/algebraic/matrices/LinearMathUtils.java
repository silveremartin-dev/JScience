...
/**
 * The linear math library. This class cannot be subclassed or instantiated
...
//why don't we have solve method for Complex matrices ???
public final class LinearMathUtils extends Object {
    /**
     * DOCUMENT ME!
     */
    private final static int EIGEN_MAX_ITERATIONS = 250;

    /**
     * Creates a new LinearMathUtils object.
     */
    private LinearMathUtils() {
    }
     ...
     */
    public static AbstractDoubleVector solve(
        final AbstractDoubleSquareMatrix M, final AbstractDoubleVector v) {
        ...
        for (i = 0; i < n; i++) {
            sum = v.getPrimitiveElement(pivot[i]);

            for (j = 0; j < i; j++)
                sum -= (lu[0].getPrimitiveElement(i, j) * array[j]);

            array[i] = sum / lu[0].getPrimitiveElement(i, i);
        }
        ...
        for (i = n - 1; i >= 0; i--) {
            ...
            for (j = i + 1; j < n; j++)
                sum -= (lu[1].getPrimitiveElement(i, j) * array[j]);

            array[i] = sum / lu[1].getPrimitiveElement(i, i);
        }
    ...
    private static double[] solveQR(final double[][] m, final double[] v) {
        ...
        for (i = 0; i < n; i++) {
            ...
            for (j = 0; j < n; j++)
                sum += (lu[0].getPrimitiveElement(j, i) * v[j]);
    ...
    /**
     * Solves the unsymmetric linear system Ax=b using the Generalized
     * Minimum Residual method (doesn't require A to be nonsingular). While
     * slower than LU decomposition, it is more robust and should be used with
     * large matrices. It is guaranted to converge exactly in N iterations for
     * an N by N matrix (minus some numerical errors).
     *
     * @param A DOCUMENT ME!
     * @param b DOCUMENT ME!
     ...
     * @return DOCUMENT ME!
     *
     * @throws MaximumIterationsExceededException If it cannot converge
     *         according to the given parameters.
     * @throws IllegalArgumentException If either the tolerance or the number
     *         of iterations is not positive. Also, if an unexpected error
     *         occurs.
     */
    public static AbstractDoubleVector solveGMRes(
        final AbstractDoubleMatrix A, final AbstractDoubleVector b,
        int max_iter, double tol) throws MaximumIterationsExceededException {
        if (max_iter <= 0) {
            throw new IllegalArgumentException(
                "Number of allowed iterations must be a positive integer: " +
                max_iter + " <= 0.");
        }

        if (tol < 0) {
            throw new IllegalArgumentException(
                "Tolerance must be positive or zero: " + tol + " < 0.");
        }
        ...
        AbstractDoubleVector x = new DoubleVector(A.numRows());
        double normb = b.norm();
        ...
        while (j <= max_iter) {
            ...
            for (i = 0; (i < m) && (j <= max_iter); i++, j++) {
                ...
                for (k = 0; k < i; k++) {
                    rotmp = applyPlaneRotation(H[k][i], H[k + 1][i], cs[k][0],
                            cs[k][1]);
                    H[k][i] = rotmp[0];
                    H[k + 1][i] = rotmp[1];
                }
                ...
                rotmp = applyPlaneRotation(H[i][i], H[i + 1][i], cs[i][0],
                        cs[i][1]);
                H[i][i] = rotmp[0];
                H[i + 1][i] = rotmp[1];
        ...
        tol = resid;
        throw new MaximumIterationsExceededException("(tol) " + tol +
            ". It doesn't converge in " + max_iter +
            "iterations. Try raising the number of allowed iterations or raising the tolerance.");
    }
     ...
     * @return DOCUMENT ME!
     */
    private static double[] applyPlaneRotation(double dx, double dy, double cs,
        double sn) {
     ...
     * @return DOCUMENT ME!
     */
    private static AbstractDoubleVector update(AbstractDoubleVector x, int k,
        double[][] H, double[] s, AbstractDoubleVector[] v) {
/************************************************
    ...
    // LEAST SQUARES
    /**
     * Fits an nth degree polynomial to data using the method of least
     * squares.
     *
     ...
     * @return DOCUMENT ME!
     */
    public static DoublePolynomial leastSquaresFit(int n, final double[][] data) {
        int i;
        int j;
        int nm1 = n++;
        double xsum;
        ...
        return new DoublePolynomial(solveCholesky(mArray, vArray));
    }

    // LINEAR REGRESSION
    /**
     * Fits a line to multi-dimensional data using the method of least
     * squares.
     *
     * @param data [0...n-1][] contains the x-series' (they must be linearly
     *        uncorrelated), [n][] contains the y-series.
     *
     * @return a vector containing the coefficients (zero component is the
     *         intercept, the rest are gradient components). E.g. y(x1, x2,
     *         ...) = coeffs(0) + coeffs(1)  x1 + coeffs(2)  x2 + ...
     */
     ...
     */
    public static AbstractDoubleVector[] orthonormalize(
        AbstractDoubleVector[] vecs) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < i; j++)
                orthovecs[i] = orthovecs[i].subtract(orthovecs[j].scalarMultiply(
                            orthovecs[j].scalarProduct(vecs[i])));
     ...
     *
     * @throws MaximumIterationsExceededException If it takes too many
     *         iterations to determine an eigenvalue.
     */
    public static double[] eigenvalueSolveHermitian(
        final AbstractComplexSquareMatrix matrix)
    ...
    /**
     * This method finds the eigenvalues and eigenvectors of a
     * Hermitian matrix.
     *
     * @param matrix a Hermitian matrix.
     * @param eigenvector an empty array of complex vectors to hold the
     *        eigenvectors. All eigenvectors will be orthogonal.
     *
     * @return an array containing the eigenvalues.
     *
     * @throws MaximumIterationsExceededException If it takes too many
     *         iterations to determine an eigenvalue.
     */
    public static double[] eigenSolveHermitian(
        final AbstractComplexSquareMatrix matrix,
        final AbstractComplexVector[] eigenvector)
        throws MaximumIterationsExceededException {
        final int n = matrix.numRows();
        final double[][] matrix2 = new double[2 * n][2 * n];
        int i;
    ...
    /**
     * This method finds the eigenvalues of a symmetric tridiagonal
     * matrix by the QL method. It is based on the NETLIB algol/fortran
     * procedure tql1 by Bowdler, Martin, Reinsch and Wilkinson.
     *
     ...
     *         iterations to determine an eigenvalue.
     */
    public static double[] eigenvalueSolveSymmetric(
        final DoubleTridiagonalMatrix matrix)
        throws MaximumIterationsExceededException {
        final int n = matrix.numRows();
        final int nm1 = n - 1;
        final double[] eigenvalue = new double[n];
        ...
        for (int i = 0; i < nm1; i++) {
            eigenvalue[i] = matrix.getPrimitiveElement(i, i);
            offdiag[i] = matrix.getPrimitiveElement(i, i + 1);
        }

        eigenvalue[nm1] = matrix.getPrimitiveElement(nm1, nm1);
        offdiag[nm1] = 0.0;
    ...
    /**
     * This method finds the eigenvalues and eigenvectors of a
     * symmetric tridiagonal matrix by the QL method. It is based on the
     * NETLIB algol/fortran procedure tql2 by Bowdler, Martin, Reinsch and
     * Wilkinson.
     *
     * @param matrix a double symmetric tridiagonal matrix.
     * @param eigenvector an empty array of double vectors to hold the
     *        eigenvectors. All eigenvectors will be orthogonal.
     *
     * @return an array containing the eigenvalues.
     *
     ...
     *         iterations to determine an eigenvalue.
     */
    public static double[] eigenSolveSymmetric(
        final DoubleTridiagonalMatrix matrix,
        final AbstractDoubleVector[] eigenvector)
        throws MaximumIterationsExceededException {
        final int n = matrix.numRows();
        final int nm1 = n - 1;
        final double[] eigenvalue = new double[n];
     ...
     * The matrix is reduced to tridiagonal form and then the QL method is
     * applied. It is based on the NETLIB algol/fortran procedure tred1/tql1
     ...
     */
    public static double[] eigenvalueSolveSymmetric(
        final AbstractDoubleSquareMatrix matrix)
        throws MaximumIterationsExceededException {
        final int n = matrix.numRows();
        final double[] eigenvalue = new double[n];
        final double[] offdiag = new double[n];
        ...
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++)
                array[i][j] = matrix.getPrimitiveElement(i, j);
        }
    ...
    /**
     * This method finds the eigenvalues and eigenvectors of a
     * symmetric square matrix. The matrix is reduced to tridiagonal form and
     * then the QL method is applied. It is based on the NETLIB algol/fortran
     * procedure tred2/tql2 by Bowdler, Martin, Reinsch and Wilkinson.
     *
     * @param matrix a double symmetric square matrix.
     * @param eigenvector an empty array of double vectors to hold the
     *        eigenvectors. All eigenvectors will be orthogonal.
    ...
    public static double[] eigenSolveSymmetric(
        ...
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++)
                transf[i][j] = matrix.getPrimitiveElement(i, j);
        }
     ...
     * @param offdiag DOCUMENT ME!
     *
     * @throws MaximumIterationsExceededException DOCUMENT ME!
     */
    private static void eigenvalueSolveSymmetricTridiagonalMatrix(
        final double[] diag, final double[] offdiag)
        throws MaximumIterationsExceededException {
                ...
                if (m != l) {
                    if (iteration++ == EIGEN_MAX_ITERATIONS) {
                        throw new MaximumIterationsExceededException(
                            "No convergence after " + EIGEN_MAX_ITERATIONS +
                            " iterations.");
                    }
     ...
     * @param diag DOCUMENT ME!
     * @param offdiag DOCUMENT ME!
     */
    private static void reduceSymmetric1_SquareToTridiagonal(
        final double[][] matrix, final double[] diag, final double[] offdiag) {
        final int n = diag.length;
        int i;
        int j;
        int k;
    ...
    private static void eigenSolveSymmetricTridiagonalMatrix(
        final double[] diag, final double[] offdiag, final double[][] transf)
        throws MaximumIterationsExceededException {
        final int n = diag.length;
        final int nm1 = n - 1;
        int m;
     ...
     * @param offdiag DOCUMENT ME!
     */
    private static void reduceSymmetric2_SquareToTridiagonal(
        final double[][] matrix, final double[] diag, final double[] offdiag) {
        final int n = diag.length;
        int i;
        int j;
