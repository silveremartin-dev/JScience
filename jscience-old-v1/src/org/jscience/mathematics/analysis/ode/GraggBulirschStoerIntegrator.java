 ...
 * fortran code by E. Hairer and G. Wanner.
 * </p>
 *
 * @author L. Maisonobe
 * @version $Id: GraggBulirschStoerIntegrator.java,v 1.2 2007-10-21 17:38:17 virtualcall Exp $
 */

public class GraggBulirschStoerIntegrator
        extends AdaptiveStepsizeIntegrator implements Named {
    ...
    public void integrate(FirstOrderDifferentialEquations equations,
        ...
        if (equations.getDimension() != y0.length) {
            throw new IntegrationException("dimensions mismatch: "
                    + "ODE problem has dimension "
                    + equations.getDimension()
                    + ", state vector has dimension "
                    + y0.length);
        }
        if (Math.abs(t - t0) <= 1.0e-12 * Math.max(Math.abs(t0), Math.abs(t))) {
            throw new IntegrationException("too small integration interval: length = "
                    + Math.abs(t - t0));
        }
        ...
        while (!lastStep) {
            ...
            if (denseOutput && !reject) {
                ...
                if (mu >= 0) {
                    ...
                    gbsInterpolator.computeCoefficients(k, mu, h);
