...
/**
     ...
     * distribuitons using a given array of probabilities as the mixing
     * parameters.
     *
...
/**
     * This special constructor creates the mixture of two distributions using
     * a specified number and its complement as the mixing probabilities.
     *
     * @param d0 the index 0 distributions
     * @param d1 the index 1 distributions
     * @param a  the index 1 mixing parameter (the index 0 parameter is 1
     *           &minus; a)
     */
...
/**
     ...
     * distributions with equal mixing parameters. The result, of course, is
     * simply another standard normal distribution.
     */
    ...
    /**
     * This method sets up the domain of the general mixture
     * distributions in terms of the distributions being mixed.
     *
    ...
    public void setParameters(Distribution[] d, double[] p) {
        ...
        int t0 = distributions[0].getType();
        int t;
    ...
    /**
     * This method sets up the domain of for the mixture of two
     * distributions.
     *
     * @param d0 the index 0 distributions
     * @param d1 the index 1 distributions
    ...
    /**
     * This method computes the density function of the mixture
     * distributions as a linear combination of the densities of the given
    ...
    public double getDensity(double x) {
        ...
        for (int i = 0; i < n; i++)
            d = d + (probabilities[i] * distributions[i].getDensity(x));

        return d;
    }

    /**
     * This method computes the cumulative distributions function of
     * the mixture distributions as a linear combination of the CDFs of the
    ...
    public double getCDF(double x) {
        ...
        for (int i = 0; i < n; i++)
            sum = sum + (probabilities[i] * distributions[i].getCDF(x));

        return sum;
    }

    /**
     * This method computes the mean of the mixture distributions as a
     * linear combination of the means of the given distributions, using the
    ...
    public double getMean() {
        ...
        for (int i = 0; i < n; i++)
            sum = sum + (probabilities[i] * distributions[i].getMean());

        return sum;
    }

    /**
     * This method computes the variance of the mixture distributions
     * in terms of the variances and means of the given distributions and the
     * mixing parameters.
     *
     * @return the variance
     */
    public double getVariance() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            sum = sum +
                (probabilities[i] * (distributions[i].getVariance() + (m * m)));
        }
     ...
     * This is done by selecting an index at random, according to the mixing
     * parameters, and then simulating a value from the randomly chosen
     * distributions.
     *
     * @return DOCUMENT ME!
     */
