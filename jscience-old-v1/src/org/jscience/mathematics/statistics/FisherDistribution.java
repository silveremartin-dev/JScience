...
/**
     ...
     * specified number of degrees of freedom in the numerator and
     * denominator.
     *
...
/**
    ...
    /**
     * This method sets the parameters, the degrees of freedom in the
     * numerator and denominator. Additionally, the normalizing constant and
     * default domain are computed.
     *
     * @param n the numerator degrees of freedom
     * @param d the denominator degrees of freedom
     */
    public void setParameters(int n, int d) {
        ...
        numeratorDegrees = n;
        denominatorDegrees = d;
        num = (double) numeratorDegrees;
        den = (double) denominatorDegrees;

        //Compute normalizing constant
        c = StatisticsUtils.logGamma(0.5 * (num + den)) -
            StatisticsUtils.logGamma(0.5 * num) -
            StatisticsUtils.logGamma(0.5 * den) +
            (0.5 * numeratorDegrees * (Math.log(num) - Math.log(den)));

        //Compute interval
        if (denominatorDegrees <= 4) {
            upper = 20;
        } else {
    ...
    public double getDensity(double x) {
        if (x < 0) {
            return 0;
        } else if ((x == 0) & (numeratorDegrees == 1)) {
            return Double.POSITIVE_INFINITY;
        } else {
            return Math.exp((c + (((0.5 * num) - 1) * Math.log(x))) -
    ...
    public double getMaxDensity() {
        ...
        if (numeratorDegrees == 1) {
            mode = getDomain().getLowerValue();
        } else {
            mode = ((num - 2) * den) / (num * (den + 2));
        }
     ...
     * The mean is &infin; if d &le; 2 and the mean is d / (d &minus; 2) if d
     * &gt; 2.
     *
     * @return the mean
     */
    public double getMean() {
        if (denominatorDegrees <= 2) {
            return Double.POSITIVE_INFINITY;
        } else {
            return den / (den - 2);
        }
    ...
    /**
     * This method returns the variance of the distribution. The
     * variance is undefined if d &le; the variance is &infin; if 2 &lt; d
     * &le; 4; and the variance is 2 d<sup>2</sup>(n + d - 2) / [(d -
     * 2)<sup>2</sup> n (d - 4)] if d &gt; 4.
     *
     * @return the variance
     */
    public double getVariance() {
        if (denominatorDegrees <= 2) {
            return Double.NaN;
        } else if (denominatorDegrees <= 4) {
            return Double.POSITIVE_INFINITY;
        } else {
            return (2.0 * (den / (den - 2)) * (den / (den - 2)) * ((den + num) -
            2)) / (num * (den - 4));
        }
    }
    ...
    public double getMoment(int n) {
        return (StatisticsUtils.gamma((0.5 * num) + n) * StatisticsUtils.gamma((0.5 * den) -
            n) * Math.pow(den / num, n)) / (StatisticsUtils.gamma(0.5 * num) * StatisticsUtils.gamma(0.5 * den));
    }
    ...
    public double getCDF(double x) {
        ...
        if (x < 0) {
            ...
            return 1 - StatisticsUtils.betaCDF(u, 0.5 * den, 0.5 * num);
        }
    }
     ...
     * done by simulating independent variables U and V, where U is chi-square
     * n and V is chi-squared d, and then computing (U / n) / (V / d).
     *
    ...
    public String toString() {
        return "F distribution [numerator degrees of freedom = " +
        numeratorDegrees + ", denominator degrees of freedom = " +
        denominatorDegrees + "]";
    }
}
