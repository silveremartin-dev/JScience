...
/**
 * This class is models the beta distribution with specified left and right
 * parameters.
 *
...
/**
     * This general constructor creates a beta distribution with specified left
...
/**
     * This default constructor creates a beta distribution with left and right
     * parameters equal to 1. This is the uniform distribution on (0, 1).
     */
    ...
    /**
     * This method sets the parameters, computes the normalizing
     * constant c, and specifies the default domain.
     *
     * @param a the left parameter
     * @param b the right parameter
     */
    public void setParameters(double a, double b) {
        ...
        c = StatisticsUtils.logGamma(left + right) -
            StatisticsUtils.logGamma(left) - StatisticsUtils.logGamma(right);
    ...
    public double getDensity(double x) {
        if ((x < 0) | (x > 1)) {
            ...
            return Math.exp(c + ((left - 1) * Math.log(x)) +
                ((right - 1) * Math.log(1 - x)));
        }
    }
    ...
    public double getMoment(int n) {
        return StatisticsUtils.beta(left + n, right) / StatisticsUtils.beta(left,
            right);
    }
    ...
    public double getCDF(double x) {
        return StatisticsUtils.betaCDF(x, left, right);
    }
