...
/**
 * This class models the hypergeometric distribution with a specified
 * population size, sample size, and number of type 1 objects. This is the
 * distribution of the number of type 1 objects in a sample chosen without
 * replacement from a finite, two-type population.
 *
...
/**
     * This general constructor creates a new hypergeometric distribution with
...
/**
    ...
    /**
     * This method set the parameters of the distribution and computes
     * the domain.
     *
     * @param m the population size
    ...
    public void setParameters(int m, int r, int n) {
        ...
        type1Size = r;
        sampleSize = n;
        c = StatisticsUtils.combinations(populationSize, sampleSize);
        setDomain(Math.max(0, sampleSize - populationSize + type1Size),
            Math.min(type1Size, sampleSize), 1, DISCRETE);
    }

    /**
     * This method computes the probability density function. The PDF
     * is f(x) = C(r, x) C(m &minus; r, n &minus; x) / C(m, n).
     *
    ...
    public double getDensity(double x) {
        ...
        return (StatisticsUtils.combinations(type1Size, k) * StatisticsUtils.combinations(populationSize -
            type1Size, sampleSize - k)) / c;
    }

    /**
     * This method computes the aximum value of the probability density
     * function. The mode occurs at (n + 1)(r + 1)/(m + 2).
     *
     * @return the maximum value of the probability density function
     */
    public double getMaxDensity() {
        double mode = Math.floor(((double) (sampleSize + 1) * (type1Size + 1)) / (populationSize +
                2));
    ...
    /**
     * This method computes the mean of the distribution, which is n r
     * / m.
     *
     * @return DOCUMENT ME!
     */
    ...
    /**
     * This method computes the variance, which is given by n (r/m) (1
     * &minus; r/m)(m &minus; n) / (m &minus; 1).
     *
     * @return the variance
     */
    public double getVariance() {
        return ((double) sampleSize * type1Size * (populationSize - type1Size) * (populationSize -
        sampleSize)) / (populationSize * populationSize * (populationSize - 1));
    }
    ...
    /**
     * This method simulate a value from the distribution. This is done
     * by simulating a sample of size n without replacment from the population
     * {1, 2, ..., m} and  counting the number of elemets less than r.
     *
    ...
    public String toString() {
        return "Hypergeometric distribution [population size = " +
        populationSize + ", sample size = " + sampleSize + ", type 1 size = " +
        type1Size + "]";
    }
}
