...
/**
 * This class models the gamma distribution with a specified shape
 * parameter and scale parameter. This distribution governs the time of an
 * event in the Poisson process.
 *
...
/**
     * This default constructor creates a new gamma distribution with shape
    ...
    public void setParameters(double k, double b) {
        ...
        c = (shape * Math.log(scale)) + StatisticsUtils.logGamma(shape);
     ...
     * If the shape parameter is &lt; 1, the maximum value is &infin; at x =
     * 0. If the shape parameter is &ge; 1, the maximum occurs at x = b(k
     * &minus 1).
     *
    ...
    public double getMoment(int n) {
        return (Math.pow(scale, n) * StatisticsUtils.gamma(n + shape)) / StatisticsUtils.gamma(shape);
    }
    ...
    /**
     * This method returns the moment generating function. This
     * function is given by M(t) = (1 &minus bt)<sup>&minus;k</sup> for t &lt;
     * 1/b.
     *
    ...
    public double getCDF(double x) {
        return StatisticsUtils.gammaCDF(x / scale, shape);
    }

    /**
     * This method simulates a value from the distribution. If shape
     * parameter is an integer, this is done by simulating an arrival time
