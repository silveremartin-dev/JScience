...
import org.jscience.mathematics.wavelet.splines.QuadraticSpline;
...
public final class MultiSpline3_5 implements Filter {
    ...
    static final double[] og = {
            5 / 1024d, 15 / 1024d, -19 / 1024d, -97 / 1024d, 13 / 512d,
            175 / 512d, -175 / 512d, -13 / 512d, 97 / 1024d, 19 / 1024d,
            -15 / 1024d, -5 / 1024d
        };

    /** DOCUMENT ME! */
    static final double[] o0 = {
            -7917 / 10240d, 2877 / 10240d, -26593 / 204800d, -1139 / 204800d,
            3951 / 102400d, 53 / 20480d, -451 / 40960d, -453 / 204800d,
            21 / 12800d, 7 / 12800d
        };

    /** DOCUMENT ME! */
    static final double[] o1 = {
            6237 / 10240d, -1197 / 10240d, -38927 / 204800d, 79779 / 204800d,
            -22911 / 102400d, -613 / 20480d, 1971 / 40960d, 2133 / 204800d,
            -81 / 12800d, -27 / 12800d
        };

    /** DOCUMENT ME! */
    static final double[] o2 = {
            -1405 / 2048d, 205 / 2048d, 5897 / 24576d, -2183 / 8192d,
            -813 / 4096d, 5455 / 12288d, -1395 / 8192d, -337 / 8192d, 9 / 512d,
            3 / 512d
        };

    /** DOCUMENT ME! */
    static final double[] on0_1 = ArrayMathUtils.scalarMultiply(-1.0,
            ArrayMathUtils.invert(o0));

    /** DOCUMENT ME! */
    static final double[] on0_2 = ArrayMathUtils.scalarMultiply(-1.0,
            ArrayMathUtils.invert(o1));

    /** DOCUMENT ME! */
    static final double[] on0_3 = ArrayMathUtils.scalarMultiply(-1.0,
            ArrayMathUtils.invert(o2));
...
/**
    ...
    public int previousDimension(int k) {
        ...
        if (((2 * (i - 2)) + 2) == k) {
            ...
            throw new IllegalScalingException(
                "Odd number of values into an even Filter. Please change the number of values/iterations: " +
                k);
        }
    }
    ...
    public static QuadraticSpline wavelet(int n0, int k) {
        ...
        if ((k > 2) && (k < (n0 - 5))) {
            v = ArrayMathUtils.padding(v.length, (2 * k) - 4, og);
        } else if (k == 0) {
            v = ArrayMathUtils.padding(v.length, 0, o0);
        } else if (k == 1) {
            v = ArrayMathUtils.padding(v.length, 0, o1);
        } else if (k == 2) {
            v = ArrayMathUtils.padding(v.length, 0, o2);
        } else if (k == (n0 - 3)) {
            ...
            v = ArrayMathUtils.padding(v.length, v.length - on0_3.length, on0_3);
        } else {
