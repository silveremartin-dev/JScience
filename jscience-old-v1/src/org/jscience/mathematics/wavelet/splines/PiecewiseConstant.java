...
/**
 * This class is used to generate piecewise constant splines to be used as
 * wavelets or related functions. It can also be used for basic interpolation.
 *
...
/**
    ...
    /**
     * Check if another object is equal to this PiecewiseConstant
     * object
     *
     * @param a DOCUMENT ME!
     *
    ...
    public boolean equals(Object a) {
        if ((a != null) && (a instanceof PiecewiseConstant) &&
                (vecteur.length == ((PiecewiseConstant) a).dimension())) {
    ...
    public int previousDimension(int k) {
        ...
        if ((2 * i) == k) {
            ...
            throw new IllegalArgumentException(
                "Odd number of values into an even Filter.");
        }
    }
    ...
    public double[] lowpass(double[] gete) {
        if (gete.length < 1) {
            throw new IllegalArgumentException(
                "The array is not long enough : " + gete.length + " < 1");
        }
    ...
    public double[] highpass(double[] gete) {
        if (gete.length < 2) {
            throw new IllegalArgumentException(
                "The array is not long enough : " + gete.length + " < 2");
        }
        ...
        for (int k = 0; k <= (dl - 1); k++) {
            sortie[k] = ArrayMathUtils.scalarProduct(ArrayMathUtils.extract(k,
                        k + 1, data), vg);
        }

        sortie[dl] = ArrayMathUtils.scalarProduct(ArrayMathUtils.extract(dl -
                    vg.length + 2, dl, data),
                ArrayMathUtils.extract(0, vg.length - 2, vg));
    ...
    public double getValue(int i) {
        if ((i < 0) || (i > (vecteur.length - 1))) {
            throw new IllegalArgumentException("Incorrect parameters : " + i +
                ", " + vecteur.length);
        }
    ...
    public void setValue(int i, double d) {
        if (i < 0) {
            throw new IllegalArgumentException(
                "The parameter must be positive : " + i + " < 0");
        }
    ...
    public SumOfDiracs derive(double a, double b) {
        ...
        for (int i = 1; i < vecteur.length; i++) {
            v[i] = ((vecteur[i] - vecteur[i - 1]) * vecteur.length) / Math.abs(b -
                    a);
        }
    ...
    /**
     * Return as an array the interpolated values of the function. Will
     * return the same values as the evaluate method because the Filter is
     * interpolatory.
     *
     * @param j number of iterations
     *
