package org.jscience.mathematics.wavelet.symmlet8;
 ...
 * power of 2 a be at least of length 8. Handles boundaries by assuming
...
public final class FastSymmlet8 extends FWT {
    ...
    static final float[] scale = {
            0.0322231006040782f, -0.0126039672622638f, -0.0992195435769564f,
            0.297857795605605f, 0.803738751805386f, 0.497618667632563f,
            -0.0296355276459604f, -0.0757657147893567f
        };

    /** DOCUMENT ME! */
    static final float[] wavelet = {
            -scale[7], scale[6], -scale[5], scale[4], -scale[3], scale[2],
            -scale[1], scale[0]
        };
...
/**
    ...
    public static void transform(float[] v, int last) {
        ...
        ans[last - 3] = (v[last - 6] * wavelet[0]) +
            (v[last - 5] * wavelet[1]) + (v[last - 4] * wavelet[2]) +
            (v[last - 3] * wavelet[3]) + (v[last - 2] * wavelet[4]) +
            (v[last - 1] * wavelet[5]) + (v[0] * wavelet[6]) +
            (v[1] * wavelet[7]);
        ans[half - 3] = (v[last - 6] * scale[0]) + (v[last - 5] * scale[1]) +
            (v[last - 4] * scale[2]) + (v[last - 3] * scale[3]) +
            (v[last - 2] * scale[4]) + (v[last - 1] * scale[5]) +
            (v[0] * scale[6]) + (v[1] * scale[7]);
        ans[last - 2] = (v[last - 4] * wavelet[0]) +
            (v[last - 3] * wavelet[1]) + (v[last - 2] * wavelet[2]) +
            (v[last - 1] * wavelet[3]) + (v[0] * wavelet[4]) +
            (v[1] * wavelet[5]) + (v[2] * wavelet[6]) + (v[3] * wavelet[7]);
        ans[half - 2] = (v[last - 4] * scale[0]) + (v[last - 3] * scale[1]) +
            (v[last - 2] * scale[2]) + (v[last - 1] * scale[3]) +
            (v[0] * scale[4]) + (v[1] * scale[5]) + (v[2] * scale[6]) +
            (v[3] * scale[7]);
        ans[last - 1] = (v[last - 2] * wavelet[0]) +
            (v[last - 1] * wavelet[1]) + (v[0] * wavelet[2]) +
            (v[1] * wavelet[3]) + (v[2] * wavelet[4]) + (v[3] * wavelet[5]) +
            (v[4] * wavelet[6]) + (v[5] * wavelet[7]);
        ans[half - 1] = (v[last - 2] * scale[0]) + (v[last - 1] * scale[1]) +
            (v[0] * scale[2]) + (v[1] * scale[3]) + (v[2] * scale[4]) +
            (v[3] * scale[5]) + (v[4] * scale[6]) + (v[5] * scale[7]);
    ...
    public static void invTransform(float[] v, int last) {
        ...
        (wavelet[4] * v[ResultingLength - 1]));
        ans[3] += ((scale[5] * v[last - 1]) +
        (wavelet[5] * v[ResultingLength - 1]));
        ans[4] += ((scale[6] * v[last - 1]) +
        (wavelet[6] * v[ResultingLength - 1]));
        ans[5] += ((scale[7] * v[last - 1]) +
        (wavelet[7] * v[ResultingLength - 1]));
