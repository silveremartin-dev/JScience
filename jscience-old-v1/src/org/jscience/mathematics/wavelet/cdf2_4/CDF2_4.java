...
public final class CDF2_4 extends Multiresolution implements Filter {
    ...
    final static double[] vg = {
            3 / 64d, -3 / 32d, -1 / 4d, 19 / 32d, 45 / 32d, 19 / 32d, -1 / 4d,
            -3 / 32d, 3 / 64d
    };
    final static double[] v3 = {
            -5 / 256d, 5 / 128d, 1 / 64d, -9 / 128d, -67 / 256d, 19 / 32d,
            45 / 32d, 19 / 32d, -1 / 4d, -3 / 32d, 3 / 64d
    };
    final static double[] v2 = {
            41 / 384d, -41 / 192d, -13 / 96d, 31 / 64d, 187 / 128d, 19 / 32d,
            -1 / 4d, -3 / 32d, 3 / 64d
    };
    final static double[] v1 = {
            -241 / 768d, 241 / 384d, 245 / 192d, 105 / 128d, -93 / 256d,
            -3 / 32d, 3 / 64d
    };
    final static double[] v0 = {
            93 / 64d, 35 / 32d, -5 / 16d, -15 / 32d, 15 / 64d
    };
    ...
    public double[] highpass(double[] v) {
        ...
        for (int k = 1; k <= (dl - 1); k++) {
            sortie[k] = ArrayMathUtils.scalarProduct(ArrayMathUtils.extract(k - 1, k + 1,
                    data), phvg);
        }

        sortie[0] = ArrayMathUtils.scalarProduct(ArrayMathUtils.extract(0, 1, data), phv0);
        sortie[dl] = ArrayMathUtils.scalarProduct(ArrayMathUtils.extract(dl, dl - 1, data),
                phv0);
