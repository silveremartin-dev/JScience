...
import org.jscience.mathematics.analysis.series.FourierMathUtils;
...
/**
 * This class use the linear spline as a general model for a signal. While
 * this is a reasonnable design choice, this can certainly be overwritten if
 * necessary. Basic operations on signal are supported.
 *
 * @author Daniel Lemire
 */
public class Signal extends LinearSpline implements Cloneable {
    /** DOCUMENT ME! */
    final static double normalisation = 1.0 / MathConstants.SQRT2;

    /**
     * This epsilon is used to verify that the FFT on real numbers
     * really returns real numbers when the iFFT is applied to it. This is a
     * common safety test which could fail for very large numbers.
     */
...
/**
    ...
    /**
     * This method generates a copy of the current object with a
    ...
    /**
     * Set the Signal to the specified length scraping or padding the
     * beginning if necessary
     *
     * @param longueur DOCUMENT ME!
     */
    public void setLengthFromEnd(int longueur) {
        double[] newvec = ArrayMathUtils.setLengthFromEnd(this.evaluate(0),
                longueur);
     ...
     *
     * @param newl DOCUMENT ME!
     */
    public void resample(int newl) {
        double[] newvec = EngineerMathUtils.resample(this.evaluate(0), newl);
        this.setValues(newvec);
    }
    ...
    public void setLengthFromBeginning(int longueur) {
        double[] newvec = ArrayMathUtils.setLengthFromBeginning(this.evaluate(0),
                longueur);
        this.setValues(newvec);
    }
    ...
    public FWTCoef fwt(int J) {
        ...
        if (J < 0) {
            throw new IllegalArgumentException(
                "Cannot have a negative number of iterations.");
        }
     ...
     * @throws IllegalArgumentException DOCUMENT ME!
     */
    public FWTPacketCoef fwtPacket(int J, PrimitiveMappingND cout) {
        if (J > 20) {
            throw new IllegalArgumentException("Too many iterations.");
        }
    ...
    private double[] lowpassProject(double[] v) {
        ...
        /*
                        for(int i=0;i<l;i++) {
                                ...
                                ans[i]=ArrayMathUtils.scalarProduct(v,Eche);
                        }
        */
        return (ans);
    }
    ...
    private double[] highpassProject(double[] v) {
        ...
        /*
                        for(int i=0;i<lOnd;i++) {
                                ...
                                ans[i]=ArrayMathUtils.scalarProduct(v,Onde);
                        }
        */
        return (ans);
    }
    ...
    public double norm() {
        ...
        return (ArrayMathUtils.norm(data));
    }
    ...
    /**
     * Performs the Fourier transform. Convenience method for {@link
     * org.jscience.mathematics.FourierMathUtils#transform(double[])
    ...
    public static Complex[] fft(double[] data) {
        return FourierMathUtils.transform(data);
    }

    /**
     * Performs the Fourier transform. Convenience method for {@link
     * org.jscience.mathematics.FourierMathUtils#transform(Complex[])
     * FourierMath.transform}.
     *
     * @param data DOCUMENT ME!
     *
    ...
    /**
     * Also noted iFFT in other packages. This is the inverse to the
     * FFT.
     *
     * @param data DOCUMENT ME!
     *
    ...
    public static Complex[] fftInverse(Complex[] data) {
        return FourierMathUtils.inverseTransform(data);
    }
    ...
    public void denoiseByFFT(int k) {
        if (k < 1) {
            throw new IllegalArgumentException(
                "This parameter must be 1 or more : " + k);
        }
        ...
        if (k > (data.length - 2)) {
            if (data.length < 4) {
                throw new IllegalArgumentException(
                    "Your signal is too short to be denoised : " + data.length +
                    " < 4");
            }

            throw new IllegalArgumentException(
                "Since you signal has dimension " + data.length +
                ", the parameter must be at most : " + (data.length - 2));
        }
        ...
        for (int l = 0; l < data.length; l++) {
            ...
            if (Math.abs(tf[l].imag()) > epsilon) {
                throw new IllegalArgumentException(
                    "Complex values detected during synthesis. Please get in touch with Daniel Lemire at Daniel.Lemire@Tintin.net to report this error.");
            }
        }
    ...
    public double entropy() {
        return (EngineerMathUtils.entropy(this.evaluate(0)));
    }

    /**
     * Apply the given array as a convolution Filter and return a new
     * Signal. As one often want to compare the result to the original signal,
     * this method is "safe", that is, it won't change the current object.
     *
     * @param f an array containing the coefficients of the convolution Filter
     *
     * @return DOCUMENT ME!
     *
     * @throws IllegalArgumentException DOCUMENT ME!
    ...
    public Signal filter(double[] f) {
        ...
        if ((data.length - (f.length - 1)) <= 0) {
            throw new IllegalArgumentException(
                "Your signal is too short for this Filter : " + data.length +
                ", " + f.length);
        }
    ...
    /**
     * Apply the median Filter of a window of size 2n+1. exception
     * IllegalArgumentException if the parameter n is negative
     *
     * @param n DOCUMENT ME!
     *
     * @return DOCUMENT ME!
    ...
    public Signal medianFilter(int n) {
        if (n < 0) {
            throw new IllegalArgumentException(
                "The parameter must be positive: " + n + " < 0");
        }
        ...
        for (int k = 0; k < (data.length - (2 * n)); k++) {
            ...
            ans[k] = ArrayMathUtils.median(vtemp);
        }
    ...
    /**
     * This denoising method will identify "short peaks" in the signal
     * and take them away. Short peaks are defined from a comparison with the
     * median filtered signal. Only "significative" peaks are detected (see
     * parameter p). This method won't denoise near the boundaries. "Short"
     * refers here to the time-domain and not the amplitude. param p
     * percentage of the range (max-min) considered as a significative step
     * param n length of the peak in the time domain exception
     * IllegalArgumentException if p is not between 0 and 1 exception
     * IllegalArgumentException if the parameter n is negative
     *
     * @param p DOCUMENT ME!
     * @param n DOCUMENT ME!
    ...
    public Signal denoiseShortPeaks(double p, int n) {
        if ((p < 0) || (p > 1)) {
            throw new IllegalArgumentException(
                "The parameter p must be between 0 and 1: " + p);
        }

        double[] values = this.interpolate(0);
        double range = ArrayMathUtils.max(values) - ArrayMathUtils.min(values);
