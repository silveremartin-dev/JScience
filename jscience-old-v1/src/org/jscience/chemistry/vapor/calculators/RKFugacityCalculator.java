...
public class RKFugacityCalculator implements IFugacityCalculator {
     ...
     * DOCUMENT ME!
     */
    private final static double COEF_A = 0.42748;

    /**
     * DOCUMENT ME!
     */
    private final static double COEF_B = 0.08664;
    ...
    public double[] calculateFugacity(double T, double P, double[] Psat) {
        ...
        for (i = 0; i < numOfComps; i++) {
            a[i] = (COEF_A * MathUtils.square(Constants.R_J_PER_MOL_K) * Math.pow(components[i].Tc,
                    2.5)) / Math.sqrt(T + Constants.K_C_DIFF) / components[i].Pc / 1E5;
        ...
        capB = (bo * P * 1000) / (Constants.R_J_PER_MOL_K * (T +
            Constants.K_C_DIFF));
        capA = (ao * P * 1000) / MathUtils.square(Constants.R_J_PER_MOL_K * (T +
                Constants.K_C_DIFF));
        ...
        if (prD > 0) {
            ...
            prZ = (2 * MathUtils.cubeRoot(prR) * Math.cos(prTheta / 3)) -
                (prA / 3);

            tmpVal = (2 * MathUtils.cubeRoot(prR) * Math.cos(((2 * Math.PI) +
                    prTheta) / 3)) - (prA / 3);
            ...
            tmpVal = (2 * MathUtils.cubeRoot(prR) * Math.cos(((4 * Math.PI) +
                    prTheta) / 3)) - (prA / 3);
        ...
        for (i = 0; i < numOfComps; i++) {
            tmpVal = (ao * ((b[i] / bo) - (2 * Math.sqrt(a[i] / ao))) * Math.log((prZ +
                    capB) / prZ)) / (bo * Constants.R_J_PER_MOL_K * (T +
                Constants.K_C_DIFF));
            lnPhi = ((b[i] * (prZ - 1)) / bo) - Math.log(prZ - bo) + tmpVal;
            lnPsat = (b[i] * P * Psat[i]) / MathUtils.square(Constants.R_J_PER_MOL_K * 1E5 * (T +
                    Constants.K_C_DIFF));
