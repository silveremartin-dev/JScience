...
public class LSystem {
     ...
     *
     * @param fileValues The file to extract and store.
     */
    public void build(File fileValues) throws Exception {
        if (fileValues.canRead() == false) {//the file name is not valid
            //Log.log(fileValues.toString(), Log.NO_FILE, true);
            return;
        }
    ...
    /**
     * Extracts the value for the depth from the string, converts it to an integer and stores the value
     * in the member field.
     *
     * @param strDepth The line with the depth value.
     * @return The integer value found behing the separator.
     * @throws Exception An exception is thrown if the number can not be parsed.
     */
    ...
    /**
     * Extracts the float value from a string where the number is behind the separator sign and returns it.
     *
     * @param strLine The string with the float value behind the separator.
    ...
    /**
     * Checks if the given string is a valid axiom and stores it in the member field.
     *
     * @param strAxiom The string containing the axiom.
     * @throws Exception Is thown if the string contains invalid signs.
     */
    private void readAxiom(String strAxiom) throws Exception {
        strAxiom = getStringBehindSeparator(strAxiom);

        //the method validateSuccessor can be used here since the axiom and the successor
    ...
    /**
     * Checks if the given string contains a valid rule, stores it in a new rule instance and adds it in the
     * member vector for all rules.
     *
     * @param strRule The string containing the rule.
     * @throws Exception Is thrown if the rule is not in the right form or contains invalid signs.
     */
    private void addRule(String strRule) throws Exception {
        ...
        if ((iSignPos = strRule.indexOf(FixedPlantsDefinitions.STR_RULESIGN)) == -1) {//there is no sign wich separates the predecessor from the successor
            //Log.log(Def.STR_RULESIGN, Log.NO_RULESIGN, true);
        }

        //copy predecessor string to new variable
        String strPred = strRule.substring(0, iSignPos).trim();

        if (strPred.length() > 1 || Character.isLetter(strPred.charAt(0)) == false) {//the predecessor is more than one character long or it is not a letter
            //output error message and do not store this rule
            //Log.log(strPred, Log.INVALID_PRED, true);
        }

        //predecessor ok, continue
        ...
        //add the rule to the rulelist
        Rule ruleNew = new Rule(strPred.charAt(0), strSucc);
        //Log.debug(ruleNew.toString());
        m_vRules.add(ruleNew);
    }

    /**
     * Gets the string behind the separator. This is the string from the separator to the end of the line.
     *
     * @param strLine The string containing the separator.
    ...
    /**
     * Checks if the axiom or the successor contain invalid signs. If so an exception is thrown. Else the
     * method returns normally. All valid signs must be listed in the Def class.
     *
    ...
    /**
     * This method derivates the LSystem. Once all values as depth, axiom and all rules are stored in this class
     * a string representing the derivated LSystem can be generated. Beginning with the axiom the letters
     * will be replaced using the rules. This is repeated as many times as indicated in the depth.
     *
     * @throws Exception If the derivation process fails.
     */
    public void derivate() throws Exception {
        m_strSystem = m_strAxiom;

        for (int i = 1; i <= m_iDepth; i++)    //repeat derivation m_iDepth times
        {
            m_strSystem = replace(m_strSystem);
        }

        //Log.debug("Derivation ended");
        //Log.debug(m_strSystem);
    }

    /**
     * This method replaces all letters in the passed string with the corresponding rule. This is actually
     * one iteration in the LSystem derivation process. If no rule can be found the letter will be inserted.
     *
     * @param strSystem The system which shall be iterated.
     * @return The new string where all letters have been replaced by rules.
     * @throws Exception If the passed string contains invalid signs.
     */
    private String replace(String strSystem) throws Exception {
        String strNewSystem = new String();

        for (int i = 0; i < strSystem.length(); i++)    //parse every sign in the system
        {
            if (FixedPlantsDefinitions.STR_SIGNS.indexOf(strSystem.charAt(i)) != -1) {
                //the character at the actual position is a valid sign so copy it to the new system
                strNewSystem = strNewSystem.concat(strSystem.substring(i, i + 1));
            } else if (Character.isLetter(strSystem.charAt(i))) {
                //the character is a letter, get the rule and replace the letter
                strNewSystem = strNewSystem.concat(getSuccessor(strSystem.charAt(i)));
            } else {
                //this should not be reached since a system may only have letters or valid signs
                //Log.log( String.valueOf( strSystem.charAt(i) ), Log.INVALID_SIGN, true);
            }
        }

        //return the new system when replacement is done
        return strNewSystem;
    }

    /**
     * Searches the successor with the specified predecessor. If the rule can not be found it will return the
     * originally passed letter. This because if a letter does not represent a rule it might be a sign for a
     * step ('F' or 'f') and will be left as is.
     *
     * @param chPred The letter being the predecessor of a rule.
     * @return The successor of the rule or the passed letter if the rule can not be found.
     */
    private String getSuccessor(char chPred) {
        ...
        //if no rule can be found the oriigial letter is returned
