...
public class ScoringMatrix extends ScoringScheme {
    ...
    public ScoringMatrix(Reader input, boolean case_sensitive)
        ...
        for (in.nextToken(); in.ttype == StreamTokenizer.TT_EOL;
             in.nextToken())
            ;
        ...
        while ((in.ttype != StreamTokenizer.TT_EOF) &&
            ...
            if (in.ttype == StreamTokenizer.TT_WORD) {
                ...
                throw new InvalidScoringMatrixException("Column headers must be " +
                        "one-character codes or the special character '" +
                        INDEL_CHAR + "'.");
            }
        ...
        for (row = 0;
             (row < dimension) && (in.ttype != StreamTokenizer.TT_EOF);
             row++) {
            ...
            if (in.ttype == StreamTokenizer.TT_WORD) {
                ...
                throw new InvalidScoringMatrixException("Rows must start with an" +
                        " one-character code or the special character '" +
                        INDEL_CHAR + "'.");
            }
            ...
            for (col = 0; col < dimension; col++) {
                ...
                if (in.nextToken() != StreamTokenizer.TT_NUMBER) {
                    throw new InvalidScoringMatrixException("Invalid value at row " + (row + 1) + ", column " +
                            (col + 1) + ".");
                }
    ...
    public int scoreSubstitution(char a, char b)
        ...
        if ((r < 0) || (c < 0)) {
            throw new IncompatibleScoringSchemeException("Substitution of character " + a + " for " + b +
                    " is not defined.");
        }
     ...
     * @throws IncompatibleScoringSchemeException
     *          if this character is not recognised
     */
