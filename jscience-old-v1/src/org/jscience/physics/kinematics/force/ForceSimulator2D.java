...
public class ForceSimulator2D implements ForceSimulator {
    ...
    private ForceIntegrator integrator;
     ...
     * by default.
     */
    public ForceSimulator2D() {
        this(new RungeKuttaIntegrator2D());
    }
     ...
     */
    public ForceSimulator2D(ForceIntegrator integr) {
        integrator = integr;
        iforces = new Force[5];
        sforces = new Force[5];
        iflen = 0;
    ...
    public void clear() {
        ...
        Spring2D.SpringFactory f = Spring2D.getFactory();
        while ( siter.hasNext() )
            f.reclaim((Spring2D)siter.next());
        springs.clear();
    }
     ...
     */
    public void addItem(ForceItem2D item) {
        items.add(item);
    }
     ...
     */
    public boolean removeItem(ForceItem2D item) {
     ...
     */
    public Spring2D addSpring(ForceItem2D item1, ForceItem2D item2) {
     ...
     * @return the Spring added to the simulation
     */
    public Spring2D addSpring(ForceItem2D item1, ForceItem2D item2, float length) {
     ...
     * @return the Spring added to the simulation
     */
    public Spring2D addSpring(ForceItem2D item1, ForceItem2D item2, float coeff, float length) {
        if ( item1 == null || item2 == null )
            throw new IllegalArgumentException("ForceItems must be non-null");
        Spring2D s = Spring2D.getFactory().getSpring(item1, item2, coeff, length);
        springs.add(s);
        return s;
    }
    ...
    public void accumulate() {
        ...
        while ( itemIter.hasNext() ) {
            ForceItem2D item = (ForceItem2D)itemIter.next();
        ...
        while ( springIter.hasNext() ) {
            Spring2D s = (Spring2D)springIter.next();
            for ( int i = 0; i < sflen; i++ ) {
