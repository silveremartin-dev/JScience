...
public class ForceSimulator3D implements ForceSimulator {
     ...
     * by default.
     */
    public ForceSimulator3D() {
        this(new RungeKuttaIntegrator3D());
    }
     ...
     */
    public ForceSimulator3D(ForceIntegrator integr) {
        integrator = integr;
        iforces = new Force[5];
        sforces = new Force[5];
        iflen = 0;
    ...
    public void clear() {
        ...
        Spring3D.SpringFactory f = Spring3D.getFactory();
        while ( siter.hasNext() )
            f.reclaim((Spring3D)siter.next());
        springs.clear();
    }
     ...
     */
    public void addItem(ForceItem3D item) {
        items.add(item);
    }
     ...
     */
    public boolean removeItem(ForceItem3D item) {
     ...
     */
    public Spring addSpring(ForceItem3D item1, ForceItem3D item2) {
     ...
     * @return the Spring added to the simulation
     */
    public Spring addSpring(ForceItem3D item1, ForceItem3D item2, float length) {
     ...
     * @return the Spring added to the simulation
     */
    public Spring addSpring(ForceItem3D item1, ForceItem3D item2, float coeff, float length) {
        if ( item1 == null || item2 == null )
            throw new IllegalArgumentException("ForceItems must be non-null");
        Spring3D s = Spring3D.getFactory().getSpring(item1, item2, coeff, length);
        springs.add(s);
        return s;
    }
