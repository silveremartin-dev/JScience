...
public class PeakFinder extends Object {
    /**
     * Maximum separation in sigma between peaks to count them as being
     * in the same multiplet.
     */
    static final double MAX_SEP = 1.3;

    /**
     * When given the standard deviation for a gaussian peak,
     * multiplying by this gives the full width at half maximum height.
     */
...
/**
     ...
     */
    public PeakFinder(String name, int[] spectrum, double sensitivity,
        double width) {
        this.name = name;
    ...
    Multiplet[] peakFind() {
        ...
        for (int i = 0; i < filter.length; i++) {
            ...
            filter[i] = (2 * ((sigma * sigma) - (k * k))) / (Math.sqrt(Math.PI) * sigma * sigma * sigma) * Math.exp(-(k * k) / (2.0 * sigma * sigma));
        ...
        for (int i = filter_limit + 1;
                i < (spectrum.length - filter_limit - 1); i++) {
            if ((SC[i] > (sensitivity * Math.sqrt(SCC[i]))) &&
                    (SC[i] > SC[i - 1]) && (SC[i] > SC[i + 1])) { //conditions met, calculate centroid

                double posn = ((SC[i - 1] * (i - 1)) + (SC[i] * i) +
                    (SC[i + 1] * (i + 1))) / (SC[i - 1] + SC[i] + SC[i + 1]);
        ...
        for (int i = 0; i < peaks.size(); i++) {
            if (i == 0) {
                ...
                if ((peaks.getPeak(i).getPosition() -
                        peaks.getPeak(i - 1).getPosition()) > (maxSeparation * width)) {
            ...
            if (rval[i].size() > 1) {
                ...
                for (int j = 0; j < rval[i].size(); j++) {
                    if (j == 0) {
                        ...
                        double Dnext = thisPeak.getPosition() -
                            nextPeak.getPosition();
                        double Knext = Dnext * Math.exp((-Dnext * Dnext) / (4.0 * sigma * sigma)) * (1.0 -
                            ((Dnext * Dnext) / (6.0 * sigma * sigma)));
                        double psnCorrection = (nextPeak.getArea() * Knext) / thisPeak.getArea();
                        temp.addPeak(new Peak(thisPeak.getPosition() +
                                psnCorrection, thisPeak.getArea(),
                                thisPeak.getWidth()));
                    } else if (j == (rval[i].size() - 1)) {
                        Peak thisPeak = rval[i].getPeak(j);
                        Peak lastPeak = rval[i].getPeak(j - 1);
                        double Dlast = thisPeak.getPosition() -
                            lastPeak.getPosition();
                        double Klast = Dlast * Math.exp((-Dlast * Dlast) / (4.0 * sigma * sigma)) * (1.0 -
                            ((Dlast * Dlast) / (6.0 * sigma * sigma)));
                        ...
                        double Klast = Dlast * Math.exp((-Dlast * Dlast) / (4.0 * sigma * sigma)) * (1.0 -
                            ((Dlast * Dlast) / (6.0 * sigma * sigma)));
                        double psnCorrection = ((nextPeak.getArea() * Knext) +
                            (lastPeak.getArea() * Klast)) / thisPeak.getArea();
                        temp.addPeak(new Peak(thisPeak.getPosition() +
                                psnCorrection, thisPeak.getArea(),
                                thisPeak.getWidth()));
                    }
        ...
        for (int m = 0; m < rval.length; m++) {
            ...
            for (int ch = (int) Math.round(rval[m].getPeak(0).getPosition() -
                        (width * maxSeparation));
                    ch < (int) Math.round(rval[m].getPeak(rval[m].size() - 1)
                                                     .getPosition() +
                        (width * maxSeparation)); ch++) {
