 ...
 * The <code>NonLinearFit</code> class which creates and calls this must
...
public class LevenbergMarquadt {
    ...
    /**
     * smooth variation parameter between using exact solution, or a
     * small step in that direction
     */
    private double lambda = -1.0;

    /** alphada=beta is the equation to be solved. alpha and space are similar */
...
/**
     ...
     * @return an instance of <code>LevenbergMarquadt</code>
     */
     ...
     */
    public void setup(double[] counts, double[] errors, int minChannel,
        int maxChannel) {
        this.data = counts;
        this.errors = errors;
     ...
     *
     * @throws Exception trying to diagonalize singular matrices
     *
     * @see #FIRST_ITERATION
     * @see #NEXT_ITERATION
     * @see #LAST_ITERATION
    ...
    /**
     * Calculates space, vec, and chiSq from passed parameters. To do
     * this, assigns parameter values in nonLinFit
     *
     * @param params array of parameters to calculate from
     *
     * @throws FitException DOCUMENT ME!
     */
    private void calculate(Parameter[] params) throws FitException {
        ...
        for (i = 0; i < params.length; i++) {
            nonLinFit.setParameter(params[i].getName(),
            ...
            for (l = 0; l < nPar; l++) {
                if (!params[l].isFix()) {
                    ...
                    for (m = 0; m <= l; m++) {
                        if (!params[m].isFix()) {
                            k++;
                            space.element[j][k] = space.element[j][k] +
                                (weight * dyda[m]);
                        }
                    }
    ...
    /**
     * Assigns the error values in the parameters, based on the
