...
/**
 * A class that finds problems in a knowledge base.  It is not meant to be
 * instantiated.
 */
public class Diagnostics {
        ...
        while (it.hasNext()) {
            ...
            if ((forms == null) || (forms.size() < 1)) {
                ...
                for (int i = 0; i < forms.size(); i++) {
                    ...
                    if (formula.theFormula.substring(1, 14)
        ...
        while (it.hasNext()) {
            ...
            if ((forms == null) || (forms.size() < 1)) {
                ...
                for (int i = 0; i < forms.size(); i++) {
                    ...
                    if (formula.theFormula.substring(1, 9)
                                              .equalsIgnoreCase("instance") ||
                            formula.theFormula.substring(1, 9)
                                                  .equalsIgnoreCase("subclass") ||
                            formula.theFormula.substring(1, 13)
                                                  .equalsIgnoreCase("subAttribute") ||
                            formula.theFormula.substring(1, 12)
                                                  .equalsIgnoreCase("subrelation") ||
                            formula.theFormula.substring(1, 14)
                                                  .equalsIgnoreCase("subCollection")) {
                        found = true;
                    }
                }
        ...
        while (it.hasNext()) {
            ...
            if (subs.size() > 1) {
                ...
                for (int i = 0; i < subs.size(); i++) {
                    ...
                    String parent = expression.substring(expression.indexOf(
                                " ", 10) + 1, expression.indexOf(")", 10));
                    ...
                    if ((d != null) && (d.size() > 0)) {
                        for (int j = i + 1; j < parents.size(); j++) {
                            ...
                            if (d.contains(term2.intern())) {
                                result.add(term);
                                System.out.println(
                                    "INFO in childrenOfDisjointParents(): " +
                                    term1 + " and " + term2 +
                                    " are disjoint parents of " + term + ".");
                            }
                        }
                    }
                }
    ...
    /**
     * Return a list of classes that are subclasses of a partitioned
     * class, which do not appear in the partition listing.  For example,
     * (subclass E A), (partition A B C D).  "exhaustiveDecomposition" has the
     * same meaning and needs to be checked also.
     *
     * @param kb DOCUMENT ME!
     *
     * @return DOCUMENT ME!
     ...
     * @param f DOCUMENT ME!
     *
     * @return true if a quantifiers in a quantifier list is not found in the
    ...
    private static boolean quantifierNotInStatement(Formula f) {
        ...
        if (!f.car().equalsIgnoreCase("forall") && // Recurse for complex expressions.
            ...
            return (quantifierNotInStatement(f1) ||
            quantifierNotInStatement(f2));
        }
        ...
        if ((form.car() != null) && (form.car().length() > 0)) { // This test shouldn't be needed.
            ...
            if ((rest.indexOf("exists") != -1) ||
                    (rest.indexOf("forall") != -1)) { //nested quantifiers
    ...
    /**
     * Find cases where a variable appears in a quantifier list, but
     * not in the body of the quantified expression.  For example (exists
     * (?FOO) (bar ?FLOO Shmoo))
     *
     * @param kb DOCUMENT ME!
     *
    ...
    /**
     * Return a list of terms that do not ultimately subclass from
     * Entity.
     *
     * @param kb DOCUMENT ME!
     *
            ...
            if ((forms == null) || (forms.size() < 1)) {
                ...
                for (int i = 0; i < forms.size(); i++) {
                    ...
                    if (formula.theFormula.substring(1, 9)
                        ...
                        String parent = formula.theFormula.substring(formula.theFormula.indexOf(
                                    " ", 10) + 1,
                                formula.theFormula.indexOf(")", 10));
                        ArrayList parentList = (ArrayList) kb.parents.get(parent.intern());

                        if (((parentList != null) &&
                                parentList.contains("Entity")) ||
                                parent.equalsIgnoreCase("Entity")) {
                            found = true;
                        }
                    }
                }
    ...
    private static KB makeEmptyKB(String kbName) {
        ...
        String emptyCFilename = kbDir + File.separator +
     ...
     * @param pQuery DOCUMENT ME!
     * @param testType DOCUMENT ME!
     *
     * @return DOCUMENT ME!
     */
    private static String reportAnswer(KB kb, String proof, Formula query,
        String pQuery, String testType) {
        ...
        if (proof.indexOf("Syntax error detected") != -1) {
            ...
            html = html.append(query.format(kbHref, "&nbsp;", "<br>") +
                    "<br><br>");
    ...
    /**
     * Iterating through all formulas, return a proof of an
     * inconsistent or redundant one, if such a thing exists.
     *
     * @param kb DOCUMENT ME!
     *
     * @return DOCUMENT ME!
    ...
    public static String kbConsistencyCheck(KB kb) {
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                System.out.println(
                    "INFO in Diagnostics.kbConsistencyCheck(): size = " +
                    processedQueries.size());

                while (q.hasNext()) {
                    Formula f = (Formula) q.next();
                    System.out.println(
                        "INFO in Diagnostics.kbConsistencyCheck(): formula = " +
                        f.theFormula);
                    processedQuery = f.makeQuantifiersExplicit();
                    System.out.println(
                        "INFO in Diagnostics.kbConsistencyCheck(): processedQuery = " +
                        processedQuery);
                    proof = empty.inferenceEngine.submitQuery(processedQuery,
                            timeout, maxAnswers);

                    String a = reportAnswer(kb, proof, query, processedQuery,
                            "Redundancy");
                    ...
                    proof = empty.inferenceEngine.submitQuery(negatedQuery.toString(),
                            timeout, maxAnswers);
                    a = reportAnswer(kb, proof, query, negatedQuery.toString(),
                            "Inconsistency");
