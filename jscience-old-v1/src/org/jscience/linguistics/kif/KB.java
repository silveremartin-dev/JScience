...
public class KB {
    ...
    private void cacheElements(String statementType, ArrayList cached) {
        ...
        if (forms != null) {
            for (int i = 0; i < forms.size(); i++) {
                ...
                if (!formula.sourceFile.substring(formula.sourceFile.length() -
                        11, formula.sourceFile.length()).equalsIgnoreCase("_Cache.kif")) {
                    String child = formula.theFormula.substring(10,
                    ...
                    if (newParents != null) {
                        for (int j = 0; j < newParents.size(); j++) {
                            ...
                            if (newParent.indexOf("(") == -1) {
                                String newFormula = "(" + statementType + " " +
                                        child + " " + newParent + ")";
     ...
     */
    public void cache() {
        String filename = kbDir + File.separator + File.separator + this.name +
                "_Cache.kif"; // Note that the double separator shouldn't be needed.
            ...
            if ((forms != null) && (forms.size() > 0)) {
                for (int i = 0; i < forms.size(); i++) {
                    ...
                    if ((formula.theFormula.indexOf("(", 2) == -1) && // Ignore cases where parent class is a function
                            !formula.sourceFile.substring(formula.sourceFile.length() -
                                    11, formula.sourceFile.length())
                                    .equalsIgnoreCase("_Cache.kif")) {
                        if (formula.theFormula.substring(1, 9).equalsIgnoreCase("instance") ||
                                formula.theFormula.substring(1, 9)
            ...
            if ((forms != null) && (forms.size() > 0)) {
                for (int i = 0; i < forms.size(); i++) {
                    ...
                    if ((formula.theFormula.indexOf("(", 2) == -1) &&
                            !formula.sourceFile.substring(formula.sourceFile.length() -
                                    11, formula.sourceFile.length())
                                    .equalsIgnoreCase("_Cache.kif")) { // Ignore cases where parent class is a function
     ...
     */
    public ArrayList askWithRestriction(int argnum1, String term1, int argnum2,
                                        String term2) {
    ...
    public ArrayList ask(String kind, int argnum, String term) {
        if (kind.compareTo("arg") == 0) {
            return (ArrayList) formulas.get(kind + "-" +
                    (new Integer(argnum)).toString() + "-" + term);
        } else {
  ...
  if (KBmanager.getMgr().getPref("cache") != null &&
      ...
      cache();  */
        ...
        // caching is too slow to perform for just one formula
    }
     ...
     */
    private void writeUserAssertion(String formula, String fname)
            throws IOException {
        FileWriter fr = null;
    ...
    public String tell(String formula) {
        ...
        try {
            ...
            while (itf.hasNext()) {
                ...
                String filename = kbDir + File.separator + this.name +
                ...
                if (!constituents.contains(filename)) {
                    System.out.println("INFO in KB.tell(): Adding file: " +
                            filename + " to: " + constituents.toString());
...
cache();        */
        ...
        // caching is currently not efficient enough to invoke it after every assertion
        return "";
    }
    ...
    public ArrayList getNearestRelations(String term) {
        term = Character.toUpperCase(term.charAt(0)) +
    ...
    public ArrayList getNearestNonRelations(String term) {
        term = Character.toLowerCase(term.charAt(0)) +
                term.substring(1, term.length());
    ...
    private void reloadFormatMaps(String lang) {
        ...
        if (col == null) {
            System.out.println("Error in KB.getFormatMap(): No relation formatting file loaded for language: " +
                    lang);
        ...
        while (ite.hasNext()) {
            ...
            if (strFormat.substring(8, 10).compareTo(l) == 0) {
                ...
                String key = strFormat.substring(after + 9, theFirstQuote - 1)
                        .trim();
                String format = strFormat.substring(theFirstQuote + 1,
                        theSecondQuote).trim();

                if (format.indexOf("$") < 0) {
                    format = format.replaceAll("\\x26\\x25",
                            "\\&\\%" + key + "\\$");
                }
        ...
        if (col == null) {
            System.out.println("Error in KB.getTermFormatMap(): No term formatting file loaded for language: " +
                    lang);
        ...
        while (ite.hasNext()) {
            ...
            if (strFormat.substring(12, 14).compareTo(l) == 0) {
                ...
                int theFirstQuote = strFormat.indexOf("\"");
                int theSecondQuote = strFormat.indexOf("\"", theFirstQuote + 1);
                String key = strFormat.substring(after + 13, theFirstQuote - 1)
                        .trim().intern();
                String format = strFormat.substring(theFirstQuote + 1,
                        theSecondQuote).trim();
        ...
        for (int i = 0; i < constituents.size(); i++) {
            if (((String) constituents.get(i)).indexOf(_userAssertionsString) != -1) {
                ...
                try {
                    ...
                    System.out.println("Error in KB.deleteUserAssertions: Error writing configuration: " +
                            ioe.getMessage());
                }
    ...
    public void addConstituent(String filename)
        ...
        if (constituents.contains(filename.intern())) {
            return;
        }
        ...
        while (it.hasNext()) { // Iterate through the formulas in the file, adding them to the KB, at the appropriate key.
            ...
            if (formulas.containsKey(key)) {
                ...
                if (list == null) {
                    throw new ParseException("Error: Bad data in existing constituents at key: " +
                            key, 0);
                }
                ...
                { // Don't add formulas to the KB that already exist in the same file.
        ...
        if (filename.substring(filename.lastIndexOf(File.separator),
                filename.length()).compareTo("_Cache.kif") != 0) {
        ...
        for (int i = 0; i < newConstituents.size(); i++) {
            try {
                ...
                System.out.println(" at line: " +
                        (new Integer(pe.getErrorOffset()).toString()));
            }
        }
    }
    ...
    public String formatDocumentation(String href, String documentation) {
        ...
        while (newFormula.indexOf("&%") != -1) {
            ...
            while (Character.isJavaIdentifierPart(newFormula.charAt(j)) &&
                    (j < newFormula.length()))
                j++;

            //System.out.println("Candidate term: " + newFormula.substring(i+2,j));
            while (!containsTerm(newFormula.substring(i + 2, j)) &&
                    (j > (i + 2)))
                j--;
            ...
            if ((term != "") && containsTerm(newFormula.substring(i + 2, j))) {
                newFormula = newFormula.substring(0, i) + "<a href=\"" + href +
                        "&term=" + term + "\">" + term + "</a>" +
    ...
    public String writeInferenceEngineFormulas(TreeSet forms)
        ...
        String inferenceEngineDir = inferenceEngine.substring(0,
                inferenceEngine.lastIndexOf(File.separator));
        String filename = inferenceEngineDir + File.separator + this.name +
                "-v.kif";
        FileWriter fr = null;
        PrintWriter pr = null;
        ...
        try {
            ...
            System.out.println("Error in KB.writeInferenceEngineFormulas(): Error writing file " +
                    filename);
        } finally {
            if (pr != null) {
                pr.close();
    ...
    private void loadVampire() {
        ...
        if (formulas.values().size() != 0) {
            ...
            try {
                ...
                System.out.println("INFO in KB.loadVampire(): writing formulas to " +
                        filename);
                inferenceEngine = new Vampire(filename);
            } catch (IOException ioe) {
                System.out.println("Error in KB.loadVampire(): " +
                        ioe.getMessage());
            }
        } else {
            try {
