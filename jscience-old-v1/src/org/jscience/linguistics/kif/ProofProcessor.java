...
public class ProofProcessor {
    ...
    public boolean equalsAnswer(int answerNum, String expectedAnswer) {
        ...
        System.out.println("INFO in ProofProcessor().equalsAnswer: answer: " +
                result.toString() + " expected answer: " + expectedAnswer);
    ...
    public String returnAnswer(int answerNum) {
        ...
        if (bindingSet.tagname.equalsIgnoreCase("proof")) {
            result = result.append("[" +
                    (String) answer.attributes.get("result") + "] ");

            return result.toString();
        }

        result = result.append("[" +
                (String) bindingSet.attributes.get("type") + "] ");
    ...
    private String removeAnswerClause(String st) {
        ...
        if (st.substring(0, 3).equalsIgnoreCase("(or")) {
            ...
            st = st.substring(0, answer - 1) +
                    st.substring(end + 2, st.length());
    ...
    public int numAnswers() {
        ...
        if (queryResponse.tagname.equalsIgnoreCase("queryResponse")) {
            ...
            System.out.println("Error in ProofProcessor.numAnswers(): Bad tag: " +
                    queryResponse.tagname);
        }
