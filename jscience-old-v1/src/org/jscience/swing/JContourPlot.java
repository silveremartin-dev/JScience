...
import org.jscience.awt.ColorScheme;
 ...
 */
public final class JContourPlot extends JDoubleBufferedComponent
...
/**
     ...
     *
     * @param z an array of the form <code>z[i][j] = f(x<sub>i</sub>,
     *          y<sub>j</sub>)</code>.
     */
    ...
    /**
     * Sets the data plotted by this JContourPlot to the specified
     * data.
     *
     * @param feed DOCUMENT ME!
     */
    ...
    public Dimension getMinimumSize() {
        ...
        return new Dimension(widthMax + (3 * Contourx) + ColorScaleWidth + 5,
            (int) Math.max((data.length * 2) + (2 * Contoury), 100));
    }

    /**
     * There is a box around the graph, this sets how wide it will be
     * in the horizontal direction.
     *
     * @param k width (in pixel)
     *
     * @throws IllegalArgumentException if the width is not at least one.
     */
    public void setContourX(int k) {
        if (k < 1) {
            throw new IllegalArgumentException(
                "This parameter must be greater than 1 : " + k + " < 1");
        }
    ...
    private void rescale() {
        ...
        for (int k = 0; k < data.length; k++) {
            ...
            largeurMax = Math.max(largeurMax,
                    (int) Math.floor(data[k].length * deltax[k]));
        }
     ...
     *
     * @throws IllegalArgumentException if one of the parameters is not between
     *         0 and 1.
     */
    public void setColor(float x1, float x2, float x3) {
        if ((x1 < 0) || (x1 > 1) || (x2 < 0) || (x2 > 1) || (x3 < 0) ||
                (x3 > 1)) {
            throw new IllegalArgumentException("Incorrect parameters : " + x1 +
                ", " + x2 + ", " + x3);
        }

        if ((x1 + x2 + x3) == 0) {
            throw new IllegalArgumentException(
                "You have chosen black at the specified color. This would generate a completly black graph. Please choose another color.");
        }
    ...
    public Color getColor(float f) {
        if ((f < 0) || (f > 1)) {
            throw new IllegalArgumentException(
                "Color are given for values between 0 and 1 : " + f);
        }
    ...
    protected void offscreenPaint(Graphics g) {
        ...
        g.drawRect(Contourx + ColorScaleWidth, Contoury - 1, largeurMax,
            hauteurMax);
        ...
        for (float y = 0; y < hauteurMax; y += (hauteurMax / 255.0f)) {
            ...
            g.fillRect(Contourx, Contoury + (int) Math.floor(y),
                ColorScaleWidth - 1, 2);
        }

        g.setColor(Color.black);
        g.drawString(MaxString, Contourx, +FontHeight + Contoury);
        g.drawString(MiddleString, Contourx,
            +Contoury + (int) Math.round((hauteurMax + FontHeight) / 2.0));
        g.drawString(MinString, Contourx, hauteurMax - 1);
        g.setClip((2 * Contourx) + ColorScaleWidth, Contoury, largeurMax - 1,
            hauteurMax - 1);

        for (int k = 0; k < data.length; k++) {
            for (int l = 0; l < data[k].length; l++) {
                g.setColor(CS.getColor(data[k][l]));
                g.fillRect((int) Math.floor(l * deltax[k]) + (2 * Contourx) +
                    ColorScaleWidth, (k * deltay) + Contoury,
                    (int) Math.floor((l + 1.0) * deltax[k]) + Contourx,
                    ((k + 1) * deltay) + Contoury);
            }

            //modifi�
            g.setColor(this.getBackground());
            g.fillRect((int) Math.floor(data[k].length * deltax[k]) +
                (2 * Contourx) + ColorScaleWidth, (k * deltay) + Contoury,
                largeurMax - 1, ((k + 1) * deltay) + Contoury);
