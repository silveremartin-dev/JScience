 ...
 * @version August, 2003
 */
public class JCoinBox extends JPanel {
    /** DOCUMENT ME! */
...
/**
     * This general constructor creates a new coin box with a specified number
     * of coins, specified preferred size, a specified probability of heads,
     * and specified head, tail, and text colors.
     *
     ...
     */
    public JCoinBox(int n, double p, int s, Color a, Color b, Color c) {
        if (s < 10) {
            s = 10;
        }
...
/**
     * This special constructor creates a new coin box with a specified number
     * of coins, probability, and size, and with default head color red, tail
     * color green, and text color white
     *
     * @param n the number of coins.
     * @param p the probability of heads
     * @param s the preferred size.
     */
    public JCoinBox(int n, double p, int s) {
        this(n, p, s, Color.red, new Color(0, 180, 0), Color.white);
    }

/**
     * This special constructor creates a new coin box with a specified number
     * of coins, and probability, and with default size 40, head color red,
     * tail color green, and text color white
     *
     * @param n the number of coins.
     * @param p the probability of heads
     */
    public JCoinBox(int n, double p) {
        this(n, p, 40, Color.red, new Color(0, 128, 0), Color.white);
    }

/**
     * This special constructor creates a new coin box with a specified number
     * of coins, and with the default probability, size, and colors.
     *
     * @param n the number of coins.
     */
...
/**
     * This default constructor creates a new coin box with 10 coins and with
     * the default probability, and colors.
     */
    public JCoinBox() {
        this(10);
    }
    ...
    /**
     * This method sets the number of coins. If the new coin count is
     * greater than the old, an appropriate number of new coins are added. If
     * the new coin count is smaller than the old, an appropriate number of
     * coins are removed. The coin count is then updated.
     *
    ...
    public void setCoinCount(int n) {
        ...
        if (n > coinCount) {
            for (int i = coinCount; i < n; i++) {
                coin = new JCoin(probability, size, headColor, tailColor,
                        textColor);
                coin.setToolTipText("Coin " + (i + 1));
                add(coin);
            }
        } else if (n < coinCount) {
            for (int i = n; i < coinCount; i++)
                remove(n);
        }
    ...
    public void setProbability(double p) {
        ...
        for (int i = 0; i < coinCount; i++)
            ((JCoin) getComponent(i)).setProbability(probability);
    }
    ...
    public void setColors(Color a, Color b, Color c) {
        ...
        for (int i = 0; i < coinCount; i++)
            ((JCoin) getComponent(i)).setColors(headColor, tailColor, textColor);
    }
    ...
    /**
     * This method tosses the coins, setting them to random values, as
     * determined by the probability of heads for each coin.
     */
    public void toss() {
        for (int i = 0; i < coinCount; i++)
            ((JCoin) getComponent(i)).toss();
    }
    ...
    public int getHeadCount() {
        ...
        for (int i = 0; i < coinCount; i++)
            headCount = headCount + ((JCoin) getComponent(i)).getValue();

        return headCount;
    }

    /**
     * This method sets the state of the all of the coins (tossed or
     * untossed). Tossed coins are shown with a label and color that depend on
     * the outcome. Untossed coins are shown unlabeled in the background
     * color.
     *
    ...
    public void setTossed(boolean b) {
        ...
        for (int i = 0; i < coinCount; i++)
            getCoin(i).setTossed(b);

        repaint();
    }

    /**
     * This method returns the state of the all of the coins (tossed or
     * untossed). Tossed coins are shown with a label and color that depend on
     * the outcome. Untossed coins are shown unlabeled in the background
     * color.
     *
