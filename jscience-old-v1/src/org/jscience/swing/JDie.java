...
/**
 * This class models a standard, 6-sided die with specified colors and
 * probabilities.
 *
 ...
 * @version August, 2003
 */
public class JDie extends JComponent implements Serializable {
    /** DOCUMENT ME! */
...
/**
     * This general constructor creates a new die with specified probabilities,
     * size, back color, and spot color.
     *
     ...
     */
    public JDie(double[] p, int s, Color bc, Color sc) {
...
/**
     * This special constructor creates a new die with and specified
     * probabilities and size, and with default back color red and spot color
     * white.
     *
     * @param p the array of probabilities
     * @param s the preferred size
     */
    public JDie(double[] p, int s) {
        this(p, s, Color.red, Color.white);
    }

/**
     * This special constructor creates a new die with and specified
     * probabilities and with default size 36 and with  back color red and
     * spot color white.
     *
     * @param p the array of probabilities
     */
...
/**
     * This general constructor creates a new die with specified probabilities
     * of special type, and with specified size and colors.
     *
     ...
     * @param sc the spot color
     */
    public JDie(int t, int s, Color bc, Color sc) {
        setPreferredSize(new Dimension(s, s));
...
/**
     * This special constructor creates a new die with specified probabilities
     * of special type, and with specified size, and with default colors red
     * and white.
     *
     * @param t the type of distribution (fair, 1-6 flat, 2-5 flat, 3-4 flat,
     *          skewed left, skewed right)
     * @param s the preferred size
     */
    public JDie(int t, int s) {
        this(t, s, Color.red, Color.white);
    }

/**
     * This special constructor creates a new die with specified probabilities
     * of special type, and with default size 36 and colors red and white.
     *
     * @param t the type of distribution (fair, 1-6 flat, 2-5 flat, 3-4 flat,
     *          skewed left, skewed right)
     */
...
/**
     * This default constructor creates a new fair die, and with default size
     * and colors.
     */
    public JDie() {
        this(FAIR);
    ...
    public void paintComponent(Graphics g) {
        ...
        int size = (int) Math.min(getSize().height, getSize().width) - 4; //Third position
        int r = (int) Math.rint((3.0 * size) / 32); //Third position
        int d1 = 2 + r; //Third position
        int d2 = (int) Math.rint((2 + (size / 2.0)) - r); //Third position
        int d3 = (2 + size) - (3 * r); //Third position
    ...
    /**
     * This method sets the value of the die to a random value, as
     * determined by the probabilities.
     */
    public void roll() {
        ...
        for (int i = 0; i < 6; i++) {
            if ((sum < p) & (p <= (sum + probabilities[i]))) {
                value = i + 1;
            }
    ...
    public void setProbabilities(double[] p) {
        ...
        if (sum == 0) {
            for (int i = 0; i < 6; i++)
                probabilities[i] = 1.0 / 6;
        } else {
            for (int i = 0; i < 6; i++)
                probabilities[i] = p[i] / sum;
        }
    }
    ...
    /**
     * This method sets the state of the die (rolled or not rolled). If
     * rolled, the die is shown with the specified back color and spot color.
     ...
     * If rolled, the die is shown with the specified back color and spot
     * color. If not rolled, an outline of the die is shown in the background
     * color.
     *
