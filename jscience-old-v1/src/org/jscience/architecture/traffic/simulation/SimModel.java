...
import org.jscience.architecture.traffic.algorithms.tlc.PosMov;
...
public class SimModel extends Model implements XMLSerializable {
    ...
    /**
     * A boolean to keep track if this simulation has already run (ivm
     * initialization)
     */
...
/**
    ...
    /**
     * Stops the simulation. This should only be called when the
     * program exits. To start a new simulation, the simulation should be
     * paused with a call to pause(), then followed by a call to reset(), and
     * finally resumed with unpause().
     */
    public void stop() {
        thread.die();
    }
    ...
    /**
     * Does 1 step in the simulation. All cars move, pedestrians get
     * squashed etc...
     */
    public void doStep() {
        curCycle++;
     ...
     */
    public void moveAllRoadusers() throws InfraException { // Line below is faster than the obvious alternative

        Enumeration lanes = ArrayUtils.getEnumeration(infra.getAllInboundLanes()
                                                           .toArray());
        Drivelane lane;
            ...
            if (!ru.didMove(curCycle)) { // Only attempt to move this RU when it hasnt already
                ...
                if ((ru_pos - ru_speed) < 0) { // Handle Roadusers that possibly can cross a Node
                    ...
                    if (node instanceof SpecialNode) { // Handle Roadusers that get to Special Nodes
                        ...
                        if ((ru_pos == 0) ||
                                (0 == lane.getPosFree(li, 0, ru_len, ru_speed,
                                    ru))) {
                            ...
                            ru.setInQueueForSign(false);
                            tlc.updateRoaduserMove(ru, lane, sign, ru_pos,
                                null, null, 0, posMovs, null);
                            ...
                            if (destLane.isLastPosFree(ru_len)) { // Check if there is room on the node
                                ...
                                try {
                                    ...
                                    tlc.updateRoaduserMove(ru, lane, sign,
                                        ru_pos, destLane, destLane.getSign(),
                                        ru.getPosition(), posMovs, destLane);
                                } catch (Exception e) {
                                    System.out.println(
                                ...
                                if (destLane.isLastPosFree(ru_len)) { // Ok now the lane that should have moved, moved so try again .........
                                    ...
                                    tlc.updateRoaduserMove(ru, lane, sign,
                                        ru_pos, lane, sign, ru.getPosition(),
                                        posMovs, null);
                                }
                            }
                        }
    ...
    /**
     * New road users are placed on the roads when necessary. When
     * roads are full, new road users are queued.
     *
     * @throws InfraException DOCUMENT ME!
     * @throws ClassNotFoundException DOCUMENT ME!
     */
    public void spawnNewRoadusers()
        throws InfraException, ClassNotFoundException {
    ...
    /**
     * A road user is placed on the given edge node. When road is full
     * the ru is queued
     *
     * @param r DOCUMENT ME!
    ...
    private SpecialNode getRandomDestination(SpecialNode[] dests,
        SpecialNode here, int ruType, DestFrequency[][] destfreqs) {
    ...
    public void load(XMLElement myElement, XMLLoader loader)
        ...
        if (savedBySim) {
            ...
            try {
                tlc = factory.getInstanceForLoad(factory.getNumberByXMLTagName(
            ...
            try {
                dp = dpFactory.getInstance(dpFactory.getNumberByXMLTagName(
                            loader.getNextElementName()));
                loader.load(this, dp);
    ...
    public class SimModelThread extends Thread {
        ...
        /**
         * Is the thread alive? If this is set to false, the thread
         * will die gracefully
         */
        private volatile boolean alive;

        /**
         * The time in milliseconds this thread sleeps after a call
         * to doStep()
         */
...
/**
        ...
        /**
         * Stops the thread. Invoked when the program exitst. This
         * method cannot be named stop().
         */
        ...
        /**
         * Invokes Model.doStep() and sleeps for sleepTime
         * milliseconds
         */
        public void run() {
            while (alive) {
