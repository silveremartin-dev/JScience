...
/**
     * Creates a new <code>TrackingView</code>. A <code>TrackingView</code> is
     ...
     * @param b DOCUMENT ME!
     */
    public void showGraph(int index, boolean b) {
        show[index] = b;
    }
    ...
    /*============================================*/
    /**
     * This method is called by the <code>notifyObservers()</code>
     * method in the <code>Model</code> representing the infrastructure.
     *
        ...
        for (int num = 0; num < MAX_TRACK; num++)
            if (show[num]) {
                ...
                for (int i = 0; i < curIndex; i++)
                    g.fillArc((int) (i * xStep) + 30,
                        h - 20 - (int) (trackData[num][i] * yStep), 2, 2, 0, 360);
            }

        // highlight a certain point and give its values in trackData
        if (highlightIndex != 0) {
            g.setColor(Color.cyan);
            g.drawLine((int) (highlightIndex * xStep) + 30, 0,
                (int) (highlightIndex * xStep) + 30, h);
            g.setColor(Color.black);
    ...
    public void setSize(int width, int height) {
        buffer = new BufferedImage(width + 1, height + 1,
    ...
    public void saveData(String filename, SimModel model)
        ...
        for (int i = 0; i < MAX_TRACK; i++)
            if (show[i]) {
                fieldsDesc += ("<" + getSourceDesc(i) + ">" +
                ((i == (MAX_TRACK - 1)) ? "" : SEP));
            }

        out.println("# Format: <Cycle>" + SEP + fieldsDesc);
        out.println("#");
    ...
    /*============================================*/
    /**
     * Listens to mouse clicks on this TrackingView.  Sets the index of
     * the graph datatable to be highlighted.
     */
            ...
            if ((e.getModifiers() & InputEvent.BUTTON3_MASK) > 0) // right-click
             {
                ...
                highlightCycle = "cycle = " +
                    (startCycle + (highlightIndex * trackRate));

                String desc;

                for (int i = 0; i < MAX_TRACK; i++) {
                    highlightText[i] = getSourceDesc(i) + " = " +
                        trackData[i][highlightIndex];

                    int dot;

                    if ((dot = highlightText[i].indexOf(".")) > -1) {
                        highlightText[i] = highlightText[i].substring(0, dot +
                                2);
                    }
                }
            }
