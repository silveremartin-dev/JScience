...
/**
 * This is the class for the sign controller. Here is decided how each sign
 * should work. Reward values are gathered for each sign, when set on 'Green'
 * asking the applied TLController. Non-TrafficLight signs can also be
 * implemented when a Q-value algorithm is implemented for the appropiate type
 * of sign. After those values are gathered, this class will select the most
 * rewarding  trafficlightconfiguration for each Node and will set the Signs
 * accordingly.
 *
...
public class SignController implements XMLSerializable {
     ...
     * time, if roadusers moving from the drivelanes behind those signs would
     * collide on the node.
     */
    ...
    /**
     * The TLcontroller is used to gather Q-values and use them to set
     * all the TLs
     */
...
/**
    ...
    public SignController(TLController t, Infrastructure i) {
        ...
        for (int j=0; j<chosenConfigs.length; j++) chosenConfigs[j]=-1; // This should be done, otherwise all chosenConfigs are 0. This leads to problems, when configuration 0 is the first time the best configuration.*/
    ...
    public void switchSigns() {
        ...
        if (num_nodes > decisions.length) {
            System.out.println("SignController switchSigns WARNING : " +
                "Less decisions than nodes !!!");
        }
    ...
    /**
     * Switch the the non-TLsigns to their appropiate value according
     * to normal traffic rules On a normal junction traffic from the right
     * gets priority
     *
    ...
    private void switchNonTrafficLights(NonTLJunction node, TLDecision[] dec) {
        ...
        try {
            ...
            System.out.println(
                "An error occured while setting non-Traffic lights");
        }
    ...
    /**
     * Switch the the TLsigns to their appropiate value The
     * configuration with highest total gain is chosen
     *
     * @param node DOCUMENT ME!
     * @param dec The decision array consists of the generated values.
     */
    public void switchTrafficLights(Junction node, TLDecision[] dec) { //System.out.println("Switching TL's on junction "+node.getId());
        ...
        if (p_index == 0) {
            possibleSC[0] = signConfs[(int) Math.round(generator.nextFloat() * (num_sc -
                    1))];
            p_index = 1;
        }
     ...
     * DOCUMENT ME!
     *
     * @param ar1 DOCUMENT ME!
     * @param ar2 DOCUMENT ME!
     *
     * @return DOCUMENT ME!
     */
