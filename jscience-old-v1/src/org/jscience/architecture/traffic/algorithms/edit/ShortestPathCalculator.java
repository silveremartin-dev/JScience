...
/**
    ...
    /**
     * Calculates all the shortest paths for each EdgeNode in the
     * Infrastructure.
     *
     * @param infra The Infrastructure of which the shortest paths should be
     *        calculated.
     *
    ...
    /**
     * Calculates the shortest paths to the given Node from each and
     * every other Node in the Infrastructure.
     *
     * @param infra The Infrastructure of which the shortest paths should be
     *        calculated.
     ...
     * @throws InfraException DOCUMENT ME!
     */
    private void calcShortestPaths(Infrastructure infra, Node exit)
        throws InfraException {
                        ...
                        if ((d[id] + lanelength) < d[newId]) // Determine if the road to the Exitnode is quicker via node
                         { // Ifso, Remove the earlier thought-up shortestpaths, and use this one.
                            ...
                            newNode.remPaths(exitId, type_arr[t],
                                (int) Math.floor(d[newId] * derivation_factor));
                         ...
                         { // add this route to the shortest paths.
                            ...
                            newNode.addShortestPath(l, exitId, type_arr[t],
                                d[newId]);
                        } else if ((d[id] + lanelength) <= (int) Math.floor(
                                    d[newId] * derivation_factor)) {
                            newNode.addShortestPath(l, exitId, type_arr[t],
                                d[id] + lanelength);
                        }
                    }
    ...
    /**
     * Find the nearest which can be choosen as an alternative
     * destination
     *
