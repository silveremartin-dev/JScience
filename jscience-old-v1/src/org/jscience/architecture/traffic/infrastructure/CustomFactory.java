 ...
 * @version 1.0 This class generates CustomRoadusers. This first version supports vehicles, drivers and passengers. However, it only really uses vehicles and drivers; passengers are just an added extra. It uses the vehicle ID and driver ID as the primary key. So if you want to create a unique custom, you need to give it a unique driver and vehicle. A vehicle has a speed, length, name and determines the roaduser type. The speed can be modified by the driver, but not the length, since trafficrules do not allow a car to become longer than x meters. A driver has a name, and it an modify the speed of the vehicle. For example, an old lady will modify the speed of a bicycle by -1. An aggresive truck driver might modify the speed of a truck by +2. The SupportedTypes array maps every RoaduserFactory type that is supported to a Custom index. The TypeVehicle array maps every Custom type index to an array of Vehicle types. The VehicleName array gives every vehicle type a name. The VehicleDriver array maps every vehicle type to a list of possible drivers. From this list is then a driver picked with a chance determined by the PersonChance table. The VehiclePassenger table maps every vehicle type to a list of possible passengers. From this list are then picked a few passengers. If it is null, no passengers are allowed. The VehicleProperties table has the following meaning: - Chance this vehicle has to appear in the simulation. - Number of these vehicles currently in the simulation. - Maximum number of these vehicles in the simulation, 0 is unlimited. - Length in blocks of the vehicle. - Graphical length of the vehicle in pixels. - Graphical width of the vehicle in pixels. - Speed of the vehicle in blocks. - Maximum number of passengers. - Roaduser type. The PersonName array gives every person a name. Might be useful :-) The PersonProperties have the following meaning: - Chance this person has to appear in the simulation. - Number of these people currently in simulation - Maximum number of these people in simulation, 0 is unlimited. Set to 1 to create unique person. - Speed modifier. The VehicleDriverName table gives every vehicle, with every possible driver a name. If any part is null, a default name is generated using the vehicle name and driver name. The VehicleDriverDescription table gives every vehicle, with every possible driver a description. If any part is null, a default description is generated using the vehicle name, driver name, passenger names, current speed and position on the map. The VehicleDriverPicture table gives every vehicle, with every possible driver a picture. If any part is null, a default picture is used. The VehicleDriverSound table gives every vehicle, with every possible driver an audio clip. If any part is null, a default clip is used.
 */
public class CustomFactory {
    ...
    protected static final int[] SupportedTypes = {
            RoaduserFactory.CAR, RoaduserFactory.BUS, RoaduserFactory.BICYCLE
        };
    ...
    protected static final String[] VehicleName = {
            "Generic Car", "Generic Bus", "Generic Bicycle", "KITT", "DeLoran",
            "Smart", "Limousine", "Limousine", "GLD Cabrio",
        };
    ...
    protected static final int[][] VehicleProperties = {
            /* 0*/ { 100, 0, 0, 2, 2, 6, 3, 3, RoaduserFactory.CAR }, /* 1*/
            { 100, 0, 0, 2, 3, 10, 3, 50, RoaduserFactory.BUS }, /* 2*/
            { 100, 0, 0, 1, 1, 2, 1, 1, RoaduserFactory.BICYCLE }, /* 3*/
            { 100, 0, 0, 2, 2, 6, 3, 1, RoaduserFactory.CAR }, /* 4*/
            { 100, 0, 0, 2, 2, 6, 3, 1, RoaduserFactory.CAR }, /* 5*/
            { 100, 0, 0, 2, 1, 3, 3, 1, RoaduserFactory.CAR }, /* 6*/
            { 100, 0, 1, 3, 5, 16, 4, 4, RoaduserFactory.CAR }, // Limo 1
                                                                /* 7*/
            { 100, 0, 1, 3, 5, 16, 4, 4, RoaduserFactory.CAR }, // Limo 2
                                                                /* 8*/
            { 100, 0, 0, 2, 2, 6, 3, 1, RoaduserFactory.CAR }, // GLD Cabrio
        };
    ...
    /** DOCUMENT ME! */
    protected static final int[][] VehiclePassenger = { null, null, null };

    /** DOCUMENT ME! */
    protected static final String[] PersonName = {
            "Johnny Bravo", "Eek the Cat", "Ash Catchem from PalletTown",
            "Micheal", "Marty McFly", "Dame van de reklame", "Pietje Puk",
            "De buurman", "Meneer de Voorzitter Jilles V.",
            "Meneer de Vice-Voorzitter Chaim Z.", "Arne", "Chaim", "Gommaar",
            "Hans Bodlaender", "Jilles", "Joep", "Marco Wiering", "Matei",
            "Matthijs", "Pepijn", "Rene", "Robert", "Sietse",
        };
    ...
    /** DOCUMENT ME! */
    protected static final String[][] VehicleDriverName = {null,
            null,
            null,
            { "Knight Rider" },
    ...
    /** DOCUMENT ME! */
    protected static final String[][] VehicleDriverDescription = {null,
            null,
            null,
    ...
    public static void removeCustom(CustomRoaduser ru) {
        ...
        if (VehicleProperties[vehicle][PROP_NR_MAX] != 0) {
            VehicleChances[vehicle] = (VehicleProperties[vehicle][PROP_NR_MAX] -
                VehicleProperties[vehicle][PROP_NR_CURRENT]) / VehicleProperties[vehicle][PROP_NR_MAX];
        } else {
            VehicleChances[vehicle] = 1.0f;
        }

        if (PersonProperties[driver][PROP_NR_MAX] != 0) {
            PersonChances[driver] = (PersonProperties[driver][PROP_NR_MAX] -
                PersonProperties[driver][PROP_NR_CURRENT]) / PersonProperties[driver][PROP_NR_MAX];
        } else {
    ...
    public static Roaduser genRoaduser(int type, Node start, Node dest, int pos)
        ...
        for (int i = 0; i < drivers.length; i++) {
            ...
            if ((pick >= chance) && (pick <= (chance + dc))) { // equal too happens never anyway
                ...
                // Don't generate this vehicle anymore until one is removed from simulation.
                if ((totalchance == dc) &&
                        (PersonProperties[driver][PROP_NR_MAX] != 0)) {
    ...
    public static String getDescription(CustomRoaduser ru) {
        ...
        return "This is " + getPersonName(ru.getDriver()) + " using vehicle " +
        getVehicleName(ru.getVehicle()) + "\n" + ru.getNumPassengers() +
