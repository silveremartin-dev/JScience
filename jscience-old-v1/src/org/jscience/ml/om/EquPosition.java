 ...
 * A equatorial position is given with three values. The <code>right
 * ascension</code> and the <code>declination</code> are used to define the
 * celestial position while both values are valid within a special reference
 * time frame (the third value).
 *
...
public class EquPosition extends SchemaElement {
    ...
    // -------------------------------------------------------------------
/**
     * Constructs a new instance of a EquPosition from a given DOM
     * target Element.<br>
     *
    ...
    public EquPosition(Node positionNode) throws SchemaException {
        ...
        if (positionValues != null) {
            if (positionValues.getLength() == 1) {
                ...
                throw new SchemaException(
                    "Position can have only one <ra> element. ");
            }
        }
        ...
        if (positionValues != null) {
            if (positionValues.getLength() == 1) {
                ...
                throw new SchemaException(
                    "Position can have only one <dec> element. ");
            }
        }
    ...
    // -------------------------------------------------------------------
    /*
      * Creates an instance of an EquPosition with a geocentric
      * reference frame (equinox J2000.0).<br>
      *
      * @param ra Angle with positiv value describing the right ascension
      * of the equatorial position
      * @param dec Angle describing the declination of the equatorial position
      * @throws IllegalArgumentException if ra or dec parameter was
      * <code>null</code> or ra value was negative
      */
    public EquPosition(Angle ra, Angle dec) throws IllegalArgumentException {
        if ((ra == null) || (dec == null)) {
            throw new IllegalArgumentException(
                "ra or dec value cannot be null. ");
        }
    ...
    /*
      ...
      * reference frame (equinox J2000.0).<br>
      *
      * @param ra String describing the right ascension
      * of the equatorial position.
     ...
     * Example:<br>
     * <code> Right ascension: 4.159876545 RAD<br>
     ...
     * Checks if this EquPosition and the given Object are equal. The given
     * object is equal with this EquPosition, if the right ascension,
     * declination and the position reference frame are equal.
     *
     * @param obj The Object to compare this EquPosition with.
     *
     * @return <code>true</code> if the given Object is an instance of
     *         EquPosition and its right ascension, declination and position
     *         reference frame are equal with this EquPosition.<br>
     *         *
     *
     * @see java.lang.Object
     */
    public boolean equals(Object obj) {
        ...
        if ((ra.equals(equPosition.getRaAngle())) &&
                (dec.equals(equPosition.getDecAngle()))) {
            return true;
        } else {
            return false;
        }
    ...
    public static String getRaString(int hours, int min, int sec) {
        return hours + EquPosition.RA_HOUR + min + EquPosition.RA_MIN + sec +
        EquPosition.RA_SEC;
    }
    ...
    public static String getDecString(int deg, int min, int sec) {
        if ((deg == 0) && ((min < 0) || ((min == 0) && (sec < 0)))) {
            return "-" + deg + EquPosition.DEC_DEG + Math.abs(min) +
            EquPosition.DEC_MIN + Math.abs(sec) + EquPosition.DEC_SEC;
        }

        return deg + EquPosition.DEC_DEG + Math.abs(min) + EquPosition.DEC_MIN +
        Math.abs(sec) + EquPosition.DEC_SEC;
    }
    ...
    // -------------------------------------------------------------------
    /**
     * Adds this EquPosition to an given parent XML DOM Element. The
     * EquPosition Element will be set as a child element of the passed
     * Element.
     *
     * @param parent The parent element for this EquPosition
     *
     * @return Returns the Element given as parameter with this EquPosition as
     *         child Element.<br>
     ...
     * This might have a strange unit for Dec values.
     *
    ...
    // -------------------------------------------------------------------    
    /**
     * Returns the right ascension of this equatorial position as
     * Angle. This might have a strange unit for RA values. Maybe getRa() is
     * what you want.
     *
    ...
    // -------------------------------------------------------------------    
    /**
     * Sets the declination of the equatorial position. Input string
     * must be of format <i>01</i>�<i>02</i>'<i>03</i>''.
     *
     * @param dec The new declination of the equatorial position
     *
     * @throws IllegalArgumentException if dec was <code>null</code> or the
     *         string was malformed
     */
    public void setDec(String dec) throws IllegalArgumentException {
        try {
            int deg = Integer.parseInt(dec.substring(0,
                        dec.indexOf(EquPosition.DEC_DEG)));
            int min = Integer.parseInt(dec.substring(dec.indexOf(
                            EquPosition.DEC_DEG) + 1,
                        dec.indexOf(EquPosition.DEC_MIN)));
            int sec = Integer.parseInt(dec.substring(dec.indexOf(
                            EquPosition.DEC_MIN) + 1,
                        dec.indexOf(EquPosition.DEC_SEC)));

            double d = 0.0;

            if (dec.startsWith("-")) {
                d = deg - ((double) min / 60.0) - ((double) sec / 3600.0);
            } else {
                d = deg + ((double) min / 60.0) + ((double) sec / 3600.0);
            }
    ...
    // -------------------------------------------------------------------    
    /**
     * Sets the right ascension of this equatorial position. Input
     * sting must be of format <i>01</i>h<i>02</i>min<i>03</i>sec
     *
     * @param ra The right ascension of the equatorial position
     *
     * @throws IllegalArgumentException if ra is <code>null</code> or the
     *         string was malformed
     */
    public void setRa(String ra) throws IllegalArgumentException {
        try {
            int hour = Integer.parseInt(ra.substring(0,
                        ra.indexOf(EquPosition.RA_HOUR)));
            int min = Integer.parseInt(ra.substring(ra.indexOf(
                            EquPosition.RA_HOUR) + 1,
                        ra.indexOf(EquPosition.RA_MIN)));
            int sec = Integer.parseInt(ra.substring(ra.indexOf(
                            EquPosition.RA_MIN) + 1,
                        ra.indexOf(EquPosition.RA_SEC)));

            double r = (hour * 15) + ((double) min / 60.0 * 15) +
                ((double) sec / 3600.0 * 15);
    ...
    // -------------------------------------------------------------------    
    /**
     * Sets the right ascension of this equatorial position. The Angles
     * value has to be positive.
     *
     * @param ra The right ascension of the equatorial position
     *
     * @throws IllegalArgumentException if ra is <code>null</code> or ra value
     *         is negative
     */
    ...
    // -------------------------------------------------------------------    
    /**
     * Returns the position reference frame of this equatorial
     * position.
     *
     ...
     * If <code>null</code> is passed the new position reference frame that is
     * set is geocentric with equinox J2000.0
     *
