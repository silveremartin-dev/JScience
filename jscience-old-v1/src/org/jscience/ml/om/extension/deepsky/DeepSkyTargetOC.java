...
/**
 * DeepSkyTargetOC extends the org.jscience.ml.om.extension.deepsky.DeepSkyTarget class.<br>
...
public class DeepSkyTargetOC extends DeepSkyTarget {
    ...
    // class
    private static final String XML_XSI_TYPE_VALUE = "fgca:deepSkyOC";
    ...
    /**
     * Constructs a new instance of a DeepSkyTargetOC from a given DOM
     * target Element.<br>
     ...
     */
    public DeepSkyTargetOC(Node targetElement,
                           IObserver[] observers)
        ...
        children = target.getElementsByTagName(DeepSkyTargetOC.XML_ELEMENT_CLUSTERCLASSIFICATION);
        String cclass = null;
        if (children != null) {
            if (children.getLength() == 1) {
                child = (Element) children.item(0);
                cclass = child.getFirstChild().getNodeValue();
                this.setClusterClassification(cclass);
     ...
     *
     * @return The amount of stars in the open cluster.
     ...
     *
     * @return The classification of the open cluster (according to Trumpler)
     ...
     *
     * @return The magnitude of the brightest star in the open cluster.
     ...
     * All passed values lower than 1 are treated as -1, which means that
     ...
     *
     * @param newBrightestStar The new magnitude of the birghtest star
     *                         in the open cluster
     */
