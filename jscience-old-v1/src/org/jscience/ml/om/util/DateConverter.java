 ...
 * the all kind of date formats.<br>
 * E.g. the ISO8601 date format (A short summary about the ISO8601 date format
 * can be accessed at the <a
 * href="http://www.w3.org/TR/NOTE-datetime">W3C</a>.), or the Julian
 * date.<br>
 *
...
public class DateConverter {
     ...
     *
     * @param gregorianCalendar The gregorianDate date
     *
    ...
    public static double toJulianDate(Calendar gregorianCalendar) {
        if (gregorianCalendar == null) {
            throw new IllegalArgumentException(
                "Gregorian date has illegal value. (NULL)");
        }
        ...
        // Hour and minute value of julian date  
        double hourAndMinutes = day - 1 + ((double) (hour - tzOffset) / 24) +
            ((double) minute / 1440) + ((double) seconds / 86400);

        // Calculation of leap year  
        double leapYear = Double.NaN;
        Calendar gregStartDate = Calendar.getInstance();
        gregStartDate.set(1583, 9, 16); // 16.10.1583 day of introduction of greg. Calendar. Before that there're no leap years

        if (gregorianCalendar.after(gregStartDate)) {
            leapYear = 2 - (Math.round(year / 100)) +
                Math.round((Math.round(year / 100)) / 4);
        }
        ...
        // (DaysPerJulianYear + (Year + StartOfJulianCalendar)) + (DaysPerMonth * month + 1) + hoursAndMinutes + leapYearOffset - 1524.5   
        double julianDate = (Math.round(365.25 * (year + 4716)) +
            Math.round(30.6001 * (month + 1)) + hourAndMinutes + leapYear) -
            1524.5;
    ...
    public static Calendar toGregorianDate(double julianDate) {
        return DateConverter.toGregorianDate(julianDate,
     ...
     *        <code>NULL</code> is passed GMT will be taken)
     *
    ...
    public static Calendar toGregorianDate(double julianDate, TimeZone zone) {
        if ((julianDate == Double.NaN) ||
            ...
            throw new IllegalArgumentException(
                "Julian Date has illegal value. (Value=" + julianDate + ")");
        }
        ...
        int sec = (int) Math.round((hours * 3600) -
                ((minute * 60) + (hour * 3600)));

        double leapYear100 = Math.round((onlyDays - 1867216.25) / 36524.25);
        double daysLeapYear = (onlyDays + 2 + leapYear100) -
            Math.round(leapYear100 / 4);
        ...
        double compleateYear = Math.round((completeLeapDays - 122.1) / 365.25);
        double completeDays = Math.round(365.25 * compleateYear);
        double completeMonths = Math.round((completeLeapDays - completeDays) / 30.6001);

        int day = (int) (Math.round(completeLeapDays - completeDays -
                Math.round(30.6001 * completeMonths) + onlyMinutes));

        int month = 0;

        switch ((int) completeMonths) {
        case 14:
            month = 1;
        ...
        case 1:
        case 2:
            year = (int) compleateYear - 4715; // only AD years
        ...
        default:
            ...
            if (month > 2) {
                year = (int) compleateYear - 4716; // only AD years
            }
        }

        Calendar gregorianDate = Calendar.getInstance(zone);
        gregorianDate.set(year, month - 1, day);

        // DST offset and timezone offset calculation
        int offset = (gregorianDate.get(Calendar.ZONE_OFFSET) +
            gregorianDate.get(Calendar.DST_OFFSET)) / (3600 * 1000);
        // Month-1 as January is 0 in JAVA dates/calendars
        gregorianDate.set(year, month - 1, day, hour + offset, minute, sec);
     ...
     *
     * @return A ISO8601 conform String, or <code>null</code> if the given date
     *         was <code>null</code>
     */
    ...
    // -------------------------------------------------------------------
    /**
     * Converts a String object that contains a ISO8601 conform value
     * to an java.util.Calendar object.
     *
    ...
    public static Calendar toDate(String iso8601) throws NumberFormatException {
        ...
        String secAndTZ = iso8601.substring(iso8601.indexOf(TIME_DELIMITER) +
                4, iso8601.length());
        ...
        try {
            ...
            throw new NumberFormatException(
                "Cannot generate ISO8601 date because " + step +
                " is malformed. ");
        }
    ...
    private static TimeZone createTimezone(String timeZone)
        ...
        if (hour == 0) {
            ...
            offset = (hour * 3600 * 1000) +
                ((minute * 1000 * 60) * (hour / Math.abs(hour)));
            tz = new SimpleTimeZone(offset, "");
        }
     ...
     */
    private static String setLeadingZero(int value) {
        if ((value <= 9) && (value >= -9)) {
            if (value < 0) {
