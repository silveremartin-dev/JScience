...
/**
 * A class for converting MathML to normal math. This code is licensed
...
/**
     ...
     * @param isLambda Is the MathML String a lambda string?
     ...
     */
    public static String toNormalMath(String mathML, boolean isLambda,
        boolean isRatelaw) throws Exception {
        firstPiece = true;
    ...
    private static String treeOutput(SBMLNode rootNode) {
        ...
        if (rootNode.element.toString().startsWith("@")) {
            operator = true;
        }

        String temp = rootNode.element.toString()
                                      .substring(1,
                rootNode.element.toString().length());

        if (operator) {
            if (temp.equals("piecewise")) {
                StringBuffer b = new StringBuffer("(");

                for (int i = 0; i < ((SBMLNode) rootNode).children.size();
                        i++) {
                    if (((SBMLNode) rootNode.children.get(i)).element.toString()
                                                                         .equals("piece") &&
                            (i != 0)) {
                        b.append("else");
                    }
            ...
            if (temp.equals("eq")) {
                ...
                if (((SBMLNode) rootNode).children.size() == 2) {
                    if ((((SBMLNode) rootNode.children.get(0))).element.toString()
                                                                           .equals("2")) {
                        return "(" + "sqrt" + "(" +
                        treeOutput((SBMLNode) rootNode.children.get(1)) + "))";
                    } else {
        ...
        } else if (saxConvertor.getFunctions()
                                   .containsKey(rootNode.element.toString())) {
