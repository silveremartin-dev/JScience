/*
 * $Id: OMXMLWriter.java,v 1.3 2007-10-23 18:21:27 virtualcall Exp $
 *
...
public class OMXMLWriter extends Writer {
    ...
    protected static String[] sOMObjects = {
            ...
            "OMS", "OMSTR", "OMV", "OMBVAR", "OMATP", "OMR", "OMFOREIGN"
        };
...
/**
     ...
     * @param object the OpenMath object to write out.
     */
    private void writeElement(int indent, OMObject object) {
        int index = getElementIndex(object);

        switch (index) {
        case 1:
            writeApplication(indent, (OMApplication) object);

            break;

        case 2:
            writeAttribution(indent, (OMAttribution) object);

            break;

        case 3:
            writeByteArray(indent, (OMByteArray) object);

            break;

        case 4:
            writeBinding(indent, (OMBinding) object);

            break;

        case 5:
            writeError(indent, (OMError) object);

            break;

        case 6:
            writeFloat(indent, (OMFloat) object);

            break;

        case 7:
            writeInteger(indent, (OMInteger) object);

            break;

        case 8:
            writeSymbol(indent, (OMSymbol) object);

            break;

        case 9:
            writeString(indent, (OMString) object);

            break;

        case 10:
            writeVariable(indent, (OMVariable) object);

            break;

        case 13:
            writeReference(indent, (OMReference) object);

            break;

        case 14:
            writeForeign(indent, (OMForeign) object);
    ...
    /**
     * Write an OpenMath foreign object.<p></p>
     *
     * @param indent the level of indentation.
     * @param foreign the OpenMath foreign to write out.
     */
    private void writeForeign(int indent, OMForeign foreign) {
        writeIndent(indent);
        mPrintWriter.print("<OMFOREIGN");
        writeAttributes(foreign.getAttributes());
        mPrintWriter.print(">");
    ...
    /**
     * Write out an OpenMath reference.<p></p>
     *
     * @param indent the level of indentation.
     * @param reference the OpenMath string to write out.
     */
    private void writeReference(int indent, OMReference reference) {
        writeIndent(indent);

        mPrintWriter.print("<OMR");
        writeAttributes(reference.getAttributes());
        mPrintWriter.print("/>");
