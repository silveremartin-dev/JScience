...
/**
 * The MathMLExpression class is used by the MathMLParser to encapsulate
 * math expressions described by the <code>&lt;apply&gt;</code> tag.
 *
 * @author Mark Hale
 * @version 0.7
 */
...
/**
    ...
    private Object unaryEvaluate() {
        ...
        if (operation.equals("abs")) {
            if (value instanceof Double) {
                value = new Double(Math.abs(((Double) value).value()));
            } else if (value instanceof Complex) {
                value = new Double(((Complex) value).mod());
            }
        } else if (operation.equals("arg")) {
            if (value instanceof Double) {
                if (((Double) value).value() >= 0.0) {
                    value = DoubleField.ZERO;
                } else {
                    value = DoubleField.PI;
                }
            } else if (value instanceof Complex) {
                value = new Double(((Complex) value).arg());
            }
        } else if (operation.equals("real")) {
            if (value instanceof Complex) {
                value = new Double(((Complex) value).real());
            ...
            if (value instanceof Complex) {
                value = new Double(((Complex) value).imag());
            } else if (value instanceof ComplexVector) {
    ...
    private Object binaryEvaluate() {
        ...
        if (operation.equals("minus")) {
            ...
            if (value instanceof DoubleVector) {
                value = new Double(((DoubleVector) value).scalarProduct(
                            (DoubleVector) next));
            } else if (value instanceof ComplexVector) {
            ...
            if (value instanceof Double) {
                value = new Double(Math.pow(((Double) value).value(),
                            ((Double) next).value()));
            }
    ...
    private Object nAryEvaluate() {
        ...
        if (operation.equals("plus")) {
            ...
            value = new Double(mean());
        } else if (operation.equals("sdev")) {
            value = new Double(Math.sqrt(variance()));
        } else if (operation.equals("var")) {
            value = new Double(variance());
        } else if (operation.equals("median")) {
            double[] nums = new double[length()];
            nums[0] = ((Double) value).value();
            ...
            for (int i = 1; i < nums.length; i++) {
                ...
                nums[i] = ((Double) next).value();
            }

            value = new Double(ArrayMathUtils.median(nums));
        } else if (operation.equals("union")) {
            if (value instanceof FiniteSet) {
                FiniteSet ans = (FiniteSet) value;
                ...
                for (int i = 1; i < length(); i++) {
                    ...
                    if (next instanceof FiniteSet) {
                        ans = (FiniteSet) ans.union((FiniteSet) next);
                    }
                }
            ...
            if (value instanceof FiniteSet) {
                ...
                for (int i = 1; i < length(); i++) {
                    ...
                    if (next instanceof FiniteSet) {
                        ans = (FiniteSet) ans.intersection((FiniteSet) next);
                    }
                }
     ...
     */
    private double mean() {
        double sum = ((Double) getArgument(0)).value();
    ...
    private double variance() {
        final double m = mean();
        double num = ((Double) getArgument(0)).value();
