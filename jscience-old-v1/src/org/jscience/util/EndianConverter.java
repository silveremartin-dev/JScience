 ...
 ****************************************************************************/
//reapckaged after CVS from www.j3d.org
...
public class EndianConverter {
     ...
     */
    public static int convert(final byte[] srcBuffer, final short[] destBuffer,
                              final int srcLength, final int destOffset, final int destLength) {
        return convert(srcBuffer, destBuffer, srcLength, destOffset,
                destLength, (short) 0xff);
    }
     ...
     */
    public static int convert(final byte[] srcBuffer, final short[] destBuffer,
                              int srcLength, final int destOffset, final int destLength,
                              final short mask) {
        srcLength = Math.min(destLength * 2, (srcLength / 2) * 2);

        for (int i = 0; i < srcLength; i += 2) {
            final int tmp = ((srcBuffer[i] & 0xff) | (srcBuffer[i + 1] << 8)) &
                    mask;
     ...
     */
    public static int convert(final byte[] srcBuffer, final int[] destBuffer,
                              final int srcLength, final int destOffset, final int destLength) {
        return convert(srcBuffer, destBuffer, srcLength, destOffset,
                destLength, 0xffffffff);
    }
     ...
     */
    public static int convert(final byte[] srcBuffer, final long[] destBuffer,
                              final int srcLength, final int destOffset, final int destLength) {
        return convert(srcBuffer, destBuffer, srcLength, destOffset,
                destLength, 0xffffffffffffffffL);
    }
    ...
    public static long convert(final long value) {
        return ((value >> 56) & 0xff) | ((value >> 40) & 0xff00) |
                ...
                ((value << 8) & 0xff00000000L) | ((value << 24) & 0xff00000000L) |
     ...
     * @return number of processed bytes of srcBuffer (multiple of 4 )
     */
    public static int convertLittleEndianToFloat(final byte[] srcBuffer,
                                                 final float[] destBuffer, int srcLength, final int destOffset,
                                                 final int destLength) {
        srcLength = Math.min(destLength * 4, (srcLength / 4) * 4);

        for (int i = 0; i < srcLength; i += 4) {
            destBuffer[(i / 4) + destOffset] = Float.intBitsToFloat((srcBuffer[i] &
                    0xff) | ((srcBuffer[i + 1] << 8) & 0xff00) |
                    ((srcBuffer[i + 2] << 16) & 0xff0000) |
                    (srcBuffer[i + 3] << 24));
        }
     ...
     * @return number of processed bytes of srcBuffer (multiple of 8 )
     */
    public static int convertLittleEndianToDouble(final byte[] srcBuffer,
                                                  final double[] destBuffer, int srcLength, final int destOffset,
                                                  final int destLength) {
        srcLength = Math.min(destLength * 8, (srcLength / 8) * 8);

        for (int i = 0; i < srcLength; i += 8) {
            destBuffer[(i / 8) + destOffset] = Double.longBitsToDouble((srcBuffer[i] &
                    0xff) | ((srcBuffer[i + 1] << 8) & 0xff00) |
                    ((srcBuffer[i + 2] << 16) & 0xff0000) |
                    ((srcBuffer[i + 3] << 24) & 0xff000000) |
                    ((srcBuffer[i + 3] << 32) & 0xff00000000L) |
     ...
     *         <code>blockSize</code>)
     */
    public static int convert(final int blockSize, final byte[] srcBuffer,
                              final int[] destBuffer, final int srcLength, final int destOffset,
                              final int destLength) {
        return convert(blockSize, srcBuffer, destBuffer, srcLength, destOffset,
                destLength, 0xffffffff);
    }
    ...
    public static int convert(final int blockSize, final byte[] srcBuffer,
                              final int[] destBuffer, final int srcLength, final int destOffset,
                              final int destLength, final int mask) {
        final int length = Math.min(destLength * blockSize,
                (srcLength / blockSize) * blockSize);

        if (blockSize == 2) {
            for (int i = 0; i < length; i += 2) {
                destBuffer[(i / 2) + destOffset] = ((srcBuffer[i] & 0xff) |
                        (srcBuffer[i + 1] << 8)) & mask;
            }
     ...
     *         <code>destBuffer</code>.
     */
    public static int convertToLittleEndian(final short[] srcBuffer,
                                            final byte[] destBuffer, final int srcOffset, int srcLength) {
     ...
     *         <code>destBuffer</code>.
     */
    public static int convertToLittleEndian(final int[] srcBuffer,
                                            final byte[] destBuffer, final int srcOffset, int srcLength) {
     ...
     */
    public static int convertToLittleEndian(final int blockSize,
                                            final int[] srcBuffer, final byte[] destBuffer, final int srcOffset,
                                            int srcLength) {
     ...
     *         <code>destBuffer</code>.
     */
    public static int convertToLittleEndian(final long[] srcBuffer,
                                            final byte[] destBuffer, final int srcOffset, int srcLength) {
     ...
     *         <code>destBuffer</code>.
     */
    public static int convertToLittleEndian(final float[] srcBuffer,
                                            final byte[] destBuffer, final int srcOffset, int srcLength) {
        srcLength = Math.min(srcLength, destBuffer.length / 4);
        ...
        for (int i = 0; i < srcLength; i++) {
            final int data = Float.floatToIntBits(srcBuffer[srcOffset + i]);
     ...
     *         <code>destBuffer</code>.
     */
    public static int convertToLittleEndian(final double[] srcBuffer,
                                            final byte[] destBuffer, final int srcOffset, int srcLength) {
        srcLength = Math.min(srcLength, destBuffer.length / 8);
     ...
     *         (will be <= destLength).
     */
    public static int read(final byte[] readBuffer, final short[] destBuffer,
                           final int destOffset, final int destLength, final InputStream stream)
            throws IOException {
        return read(readBuffer, destBuffer, destOffset, destLength, stream,
                (short) 0xff);
    }
    ...
    public static int read(final byte[] readBuffer, final short[] destBuffer,
                           final int destOffset, final int destLength, final InputStream stream,
                           final short mask) throws IOException {
        ...
        while ((numOfBytesRead >= 0) && (numOfData < destLength)) {
            ...
            // without data loss
            final int maxBytesToRead = Math.min((destLength - numOfData) * 2,
                    length);

            numOfBytesRead = stream.read(readBuffer, offset,
                    maxBytesToRead - offset);

            int numOfProcessedBytes = convert(readBuffer, destBuffer,
                    numOfBytesRead + offset, destOffset + numOfData,
                    destLength - numOfData, mask);
     ...
     *         (will be <= destLength).
     */
    public static int read(final byte[] readBuffer, final int[] destBuffer,
                           final int destOffset, final int destLength, final InputStream stream)
            throws IOException {
        return read(readBuffer, destBuffer, destOffset, destLength, stream,
                0xffffffff);
    ...
    public static int read(final byte[] readBuffer, final int[] destBuffer,
        ...
        while ((numOfBytesRead >= 0) && (numOfData < destLength)) {
            ...
            // does not overflow; enables to continue reading from same stream
            // without data loss
            final int maxBytesToRead = Math.min((destLength - numOfData) * 4,
                    length);
     ...
     *         (will be <= destLength).
     */
    public static int read(final int blockSize, final byte[] readBuffer,
                           final int[] destBuffer, final int destOffset, final int destLength,
                           final InputStream stream) throws IOException {
        return read(blockSize, readBuffer, destBuffer, destOffset, destLength,
                stream, 0xffffffff);
    }
    ...
    public static int read(final int blockSize, final byte[] readBuffer,
                           final int[] destBuffer, final int destOffset, final int destLength,
                           final InputStream stream, final int mask) throws IOException {
        if (blockSize == 2) {
            return read2ByteBlock(readBuffer, destBuffer, destOffset,
                    destLength, stream, mask);
        } else if (blockSize == 3) {
            return read3ByteBlock(readBuffer, destBuffer, destOffset,
                    destLength, stream, mask);
        } else if (blockSize == 4) {
            return read(readBuffer, destBuffer, destOffset, destLength, stream,
                    mask);
    ...
    public static int read4ByteBlock(final InputStream stream)
            throws IOException {
        return (read(stream) & 0xff) | ((read(stream) << 8) & 0xff00) |
                ((read(stream) << 16) & 0xff0000) | (read(stream) << 24);
    }
    ...
    public static int read2ByteBlock(final InputStream stream)
            throws IOException {
        return (read(stream) & 0xff) | (read(stream) << 8);
    }
    ...
    public static long read8ByteBlock(final InputStream stream)
        ...
        return (read(stream) & 0xff) | ((read(stream) << 8) & 0xff00) |
                ...
                ((read(stream) << 48) & 0xff000000000000L) | (read(stream) << 56);
    }
     ...
     *                   <code>srcLength</code>.
     */
    public static void writeLittleEndian(final short[] srcBuffer,
                                         int srcOffset, int srcLength, final OutputStream stream,
                                         final byte[] tempBuffer) throws IOException {
        while (srcLength >= 0) {
            final int processedData = convertToLittleEndian(srcBuffer,
                    tempBuffer, srcOffset, srcLength);
     ...
     *                   <code>srcLength</code>.
     */
    public static void writeLittleEndian(final int[] srcBuffer, int srcOffset,
                                         int srcLength, final OutputStream stream, final byte[] tempBuffer)
            throws IOException {
        while (srcLength > 0) {
            final int processedData = convertToLittleEndian(srcBuffer,
                    tempBuffer, srcOffset, srcLength);
     ...
     *                   <code>srcLength</code>.
     */
    public static void writeLittleEndian(final long[] srcBuffer, int srcOffset,
                                         int srcLength, final OutputStream stream, final byte[] tempBuffer)
            throws IOException {
        while (srcLength > 0) {
            final int processedData = convertToLittleEndian(srcBuffer,
     ...
     *                   <code>srcLength</code>.
     */
    public static void writeLittleEndian(final int blockSize,
                                         final int[] srcBuffer, int srcOffset, int srcLength,
                                         final OutputStream stream, final byte[] tempBuffer)
            throws IOException {
        while (srcLength > 0) {
            final int processedData = convertToLittleEndian(blockSize,
                    srcBuffer, tempBuffer, srcOffset, srcLength);
     ...
     *                   <code>srcLength</code>.
     */
    public static void writeLittleEndian(final float[] srcBuffer,
                                         int srcOffset, int srcLength, final OutputStream stream,
                                         final byte[] tempBuffer) throws IOException {
        while (srcLength > 0) {
            final int processedData = convertToLittleEndian(srcBuffer,
     ...
     *                   <code>srcLength</code>.
     */
    public static void writeLittleEndian(final double[] srcBuffer,
                                         int srcOffset, int srcLength, final OutputStream stream,
                                         final byte[] tempBuffer) throws IOException {
        while (srcLength > 0) {
            final int processedData = convertToLittleEndian(srcBuffer,
     ...
     */
    public static void writeLittleEndian(final OutputStream stream,
     ...
     */
    public static void writeLittleEndian(final OutputStream stream, long value)
            throws IOException {
        for (int i = 0; i < 8; i++) {
    ...
    private static int read2ByteBlock(final byte[] readBuffer,
                                      final int[] destBuffer, final int destOffset, final int destLength,
                                      final InputStream stream, final int mask) throws IOException {
        int numOfBytesRead = 0;
        ...
        while ((numOfBytesRead >= 0) && (numOfData < destLength)) {
            ...
            // does not overflow; enables to continue reading from same stream
            // without data loss
            final int maxBytesToRead = Math.max((destLength - numOfData) * 2,
                    length);

            numOfBytesRead = stream.read(readBuffer, offset,
                    maxBytesToRead - offset);

            int numOfProcessedBytes = convert(2, readBuffer, destBuffer,
                    numOfBytesRead + offset, destOffset + numOfData,
                    destLength - numOfData, mask);
    ...
    private static int read3ByteBlock(final byte[] readBuffer,
                                      final int[] destBuffer, final int destOffset, final int destLength,
                                      final InputStream stream, final int mask) throws IOException {
        int numOfBytesRead = 0;
        ...
        while ((numOfBytesRead >= 0) && (numOfData < destLength)) {
            ...
            // does not overflow; enables to continue reading from same stream
            // without data loss
            final int maxBytesToRead = Math.max((destLength - numOfData) * 3,
                    length);

            numOfBytesRead = stream.read(readBuffer, offset,
                    maxBytesToRead - offset);

            int numOfProcessedBytes = convert(3, readBuffer, destBuffer,
                    numOfBytesRead + offset, destOffset + numOfData,
                    destLength - numOfData, mask);
