...
public class ChangeSupport {
            ...
            for (int i = 0; i < listenerCount; i++) {
                if ((listeners[i].get() == cl) && (types[i] == ct)) {
                    ...
                    System.arraycopy(listeners, i + 1, listeners, i,
                            (listenerCount - i));
                    System.arraycopy(types, i + 1, types, i, (listenerCount -
                            i));
    ...
    public void firePreChangeEvent(ChangeEvent ce) throws ChangeVetoException {
        //assert Thread.holdsLock(this) : "firePreChangeEvent must be called in a synchronized block locking the ChangeSupport";
    ...
    public void firePostChangeEvent(ChangeEvent ce) {
        //assert Thread.holdsLock(this) : "firePostChangeEvent must be called in a synchronized block locking the ChangeSupport";
        boolean needToReap = false;
