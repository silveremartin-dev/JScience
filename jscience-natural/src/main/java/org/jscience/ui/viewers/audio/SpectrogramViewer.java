 ...
 */
public class SpectrogramViewer extends VBox {
    ...
    private void renderWaveform(double[] samples) {
        double width = Math.max(800, samples.length / 50.0); // Adjust scaling factor
        ...
        for (int i = 0; i < samples.length; i++) {
            // Downsample for visual performance if needed, or just draw basic lines
            // Simple mapping: 1 sample per pixel or so. 
            // We scale 'x' based on width/samples.length
