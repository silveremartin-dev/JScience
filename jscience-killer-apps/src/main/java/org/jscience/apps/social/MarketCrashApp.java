...
/**
 * Full-Featured Market Crash Predictor Application.
 * <p>
 * Professional JavaFX application for financial market analysis using
 * technical indicators to assess crash risk.
 * </p>
 *
 ...
 * @since 1.0
 */
public class MarketCrashApp extends KillerAppBase {

    // UI Components
    private LineChart<Number, Number> priceChart;
    private XYChart.Series<Number, Number> priceSeries, smaSeries, upperBB, lowerBB;
    ...
    private VBox createChartArea() {
        ...
        // Price Chart
        Label priceTitle = new Label("📈 Price Chart");
        priceTitle.setStyle("-fx-font-size: 16px; -fx-font-weight: bold;");

        priceChart = ChartFactory.createLineChart("Market Price", "Day", "Price ($)");
        priceChart.setAnimated(false);
        priceChart.setCreateSymbols(false);

        priceSeries = ChartFactory.createSeries("Close Price");
        smaSeries = ChartFactory.createSeries("SMA-20");
        upperBB = ChartFactory.createSeries("Upper BB");
        lowerBB = ChartFactory.createSeries("Lower BB");
    ...
    private VBox createControlPanel() {
        ...
        panel.getChildren().addAll(
            ...
            statsTitle, statsBox,
            new Separator(),
            settingsTitle, smaBox, rsiBox, showSMA, showBollinger,
            new Separator(),
            alertTitle, alertLog
        );
    ...
    private VBox createSliderBox(String name, double min, double max, double initial) {
        ...
        slider.valueProperty().addListener((o, ov, nv) -> 
            label.setText(name + " " + nv.intValue()));
    ...
    private void generateSampleData() {
        ...
        for (int i = 0; i < 300; i++) {
            ...
            if (i >= 180 && i <= 220) {
                volatility = 0.06; // High volatility crash period
                price *= 0.985; // Downward bias
            } else if (i > 220 && i <= 260) {
                price *= 1.005; // Recovery
            }
            ...
            double volume = 1_000_000 + Math.random() * 500_000;

            if (i >= 180 && i <= 220) volume *= 3;
        ...
        log("Generated " + marketData.size() + " trading days of sample data");
    }
    ...
    private void updateIndicators() {
        if (currentIndex >= marketData.size()) return;
    ...
    private void updateRiskLevel(List<Candle> candles, double price, Real sma, Real rsi) {
        ...
        // Below SMA
        if (sma != null && price < sma.doubleValue() * 0.95) score += 2;
        
        // RSI extreme
        if (rsi != null) {
            double r = rsi.doubleValue();
            if (r < 25 || r > 75) score += 2;
            else if (r < 30 || r > 70) score += 1;
        }
        ...
        // Price decline
        Real pctChange = TechnicalIndicators.percentChange(candles.subList(Math.max(0, candles.size() - 20), candles.size()));
        if (pctChange != null && pctChange.doubleValue() < -10) score += 2;
